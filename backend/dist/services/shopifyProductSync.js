...
exports.syncProductWithShopify = void 0;
const shopifyHttp_1 = require("./shopifyHttp");
const shopifyProductMapper_1 = require("./shopifyProductMapper");
...
const normalizeFeatureItems = (value) => {
    if (!Array.isArray(value)) {
        return [];
    }
    return value
        ...
        if (item && typeof item === "object") {
            ...
            const description = normalizeText(record.description ?? record.value ?? record.text);
            if (!name && !description) {
                return null;
            }
            return {
...
const buildFeaturesText = (features) => features
    .map((feature) => feature.description
    ? `${feature.name} - ${feature.description}`
    : feature.name)
    .join("\n");
const buildPlansText = (plans) => {
    ...
    return plans
        ...
        const customSegments = customFields
            ...
            const fieldText = typeof field === "string"
                ? field.trim()
...
const resolveVendorName = async (product) => {
    const inlineVendorName = normalizeText(product.vendorResolved?.businessName ??
    ...
    try {
        const vendorProfileSnap = await firebase_1.firestore
        ...
        return normalizeText(vendorProfile.businessName ??
...
const normalizeProductForShopify = (product) => {
    const basic = product.vendor?.basic ?? product.basic ?? {};
    const pricing = product.vendor?.productPlanPricing ??
        product.vendor?.pricing ??
        product.pricing ??
        {};
    const media = product.vendor?.media ?? product.media ?? {};
    const verification = product.vendor?.verification ??
        product.verification ??
        {};
    const features = normalizeFeatureItems(product.vendor?.features ?? product.features);
    return {
        ...product,
        basic: {
            productName: normalizeText(basic.productName),
            category: normalizeText(basic.categoryName ?? basic.category ?? product.basic?.category),
            description: normalizeText(basic.description ?? product.basic?.description),
        },
        pricing: {
            ...pricing,
        ...
        metafieldsPayload: {
            ...
            verified: typeof verification.productVerified === "boolean"
                ? verification.productVerified
                : null,
            verifiedVendorLinkBadge: typeof verification.productLinkVerified === "boolean"
                ? verification.productLinkVerified
                : null,
            sponsored: typeof verification.isSponsored === "boolean"
                ? verification.isSponsored
                : null,
            supportResponseSlaBadge: typeof verification.supportResponseVerified === "boolean"
                ? verification.supportResponseVerified
                : null,
            refundClarityBadge: typeof verification.refundClarityVerified === "boolean"
...
if (process.env.NODE_ENV === "production") {
    if (SHOPIFY_DRY_RUN) {
        throw new Error("SHOPIFY_DRY_RUN must be false in production");
    }
}
/**
...
const unpublishProductFromAllPublications = async (productGid) => {
  ...
  `;
    const publicationsRes = await shopifyHttp_1.shopifyRest.post("/graphql.json", {
        query: queryPublications,
        variables: { id: productGid },
    });
    const publications = publicationsRes.data?.data?.node?.publications
        ?.edges ?? [];
  ...
  `;
    for (const pub of publications) {
        const res = await shopifyHttp_1.shopifyRest.post("/graphql.json", {
            query: mutation,
            variables: {
                id: productGid,
        ...
        const errors = res.data?.data?.publishableUnpublish
            ?.userErrors;
        if (errors?.length) {
            throw new Error(`Failed to unpublish from ${pub.node.name}: ` +
                errors.map((e) => e.message).join(", "));
        }
    }
};
const syncProductWithShopify = async ({ product, shopifyApiStatus, }) => {
    ...
    const shopifyProductId = shopifyData.productId ?? null;
    ...
    if (shopifyApiStatus !== "active" &&
        shopifyApiStatus !== "draft") {
        throw new Error(`Invalid shopifyApiStatus received: ${shopifyApiStatus}`);
    }
    if (shopifyApiStatus === "draft" &&
        !shopifyProductId) {
        console.log("Skipping unlist: No Shopify product exists yet");
        return { action: "skipped-no-product" };
    }
    /* ================= TEXT BUILDERS ================= */
    const { featuresText, plansText, affiliateUrl, thumbnailUrl, typeMultiple, keywords, verified, verifiedVendorLinkBadge, sponsored, supportResponseSlaBadge, refundClarityBadge, vendorId, vendorProfileUrl, productId, } = normalizedProduct.metafieldsPayload;
    const payload = (0, shopifyProductMapper_1.buildShopifyProductPayload)(normalizedProduct);
    ...
    if (shopifyProductId) {
        try {
            ...
            if (!SHOPIFY_DRY_RUN) {
                await shopifyHttp_1.shopifyRest.put(`/products/${shopifyProductId}.json`, updatePayload);
            }
            await (0, shopifyMetafields_1.setProductMetafields)({
                shopifyProductId,
                featuresText,
                plansText,
                affiliateUrl,
            ...
            if (shopifyApiStatus === "draft") {
                await unpublishProductFromAllPublications(toProductGid(shopifyProductId));
            }
            return {
    ...
    if (isActive) {
        ...
        try {
            const response = await shopifyHttp_1.shopifyRest.post("/products.json", payload);
            const createdId = extractNumericId(response.data.product.id);
...
exports.syncProductWithShopify = syncProductWithShopify;
