...
exports.syncProductRankingToShopify = void 0;
const firebaseAdmin_1 = require("../config/firebaseAdmin");
const shopifyHttp_1 = require("./shopifyHttp");
const shopifyMetafields_1 = require("./shopifyMetafields");
...
const PLAN_FALLBACK_PRIORITIES = {
    free: 1,
    starter: 2,
    business: 3,
    enterprise: 4,
...
const parseRatingValue = (value) => {
    ...
    try {
        ...
        const parsedValue = toFiniteNumber(parsed.value ?? parsed.rating ?? parsed.average);
        if (parsedValue === null) {
            return null;
        }
        const scaleMax = toFiniteNumber(parsed.scale_max) ??
...
const parseReviewCountValue = (value) => {
    const directNumber = toFiniteNumber(value);
    if (directNumber !== null) {
    ...
    try {
        const parsed = JSON.parse(value);
        const parsedValue = toFiniteNumber(parsed.count ?? parsed.value ?? parsed.review_count);
...
const getPlanScore = (plan) => PLAN_SCORES[normalizePlanKey(plan)] ?? 0;
const getPlanFallbackPriority = (plan) => PLAN_FALLBACK_PRIORITIES[normalizePlanKey(plan)] ?? 0;
const isEffectivelyZero = (value) => value === null || value === undefined || !Number.isFinite(value) || value === 0;
const extractAnalyticsGroupingHints = (data) => {
    const hints = [
...
const extractCachedMetafields = (product) => product.shopify?.shopifyData?.metafields ?? {};
const extractShopifyProductId = (product) => {
    const directId = toFiniteNumber(product.shopify?.productId) ??
...
const resolveFallbackGroupCategory = (product) => normalizeText(product.vendor?.basic?.subCategoryName) ||
    normalizeText(product.vendor?.basic?.category) ||
    normalizeText(product.shopify?.product?.category) ||
    "all-products";
const resolveGroupingKey = (candidate, shopifyInput) => candidate.groupingHints[0] ||
...
const fetchProductsById = async (productIds) => {
    const productMap = new Map();
    const productIdChunks = chunk(productIds, 250);
    for (const productIdChunk of productIdChunks) {
        const refs = productIdChunk.map((productId) => firebaseAdmin_1.firestore.collection(PRODUCTS_COLLECTION).doc(productId));
        const snapshots = await firebaseAdmin_1.firestore.getAll(...refs);
...
const fetchShopifyRankingInputs = async (shopifyProductIds) => {
  ...
  `;
    for (const productIdChunk of productIdChunks) {
        const response = await shopifyHttp_1.shopifyGraphQL.post("", {
            query,
            variables: {
                ids: productIdChunk.map((productId) => `gid://shopify/Product/${productId}`),
            },
        });
        ...
        nodes.forEach((node) => {
            ...
            const shopifyProductId = toFiniteNumber(node.legacyResourceId) ??
                toFiniteNumber(typeof node.id === "string" ? node.id.split("/").pop() : null);
            if (shopifyProductId === null) {
                return;
            }
            const roundedProductId = Math.round(shopifyProductId);
            const avgRating = parseRatingValue(node.reviewsRating?.value) ??
                parseRatingValue(node.customAvgRating?.value) ??
                0;
            const reviewCount = parseReviewCountValue(node.reviewsRatingCount?.value) ??
...
const syncProductRankingToShopify = async () => {
    ...
    const analyticsSnapshot = await firebaseAdmin_1.firestore
        .collection(ANALYTICS_COLLECTION)
        .where("success", "==", true)
        .where("range", "==", "30d")
        .get();
    const aggregatedAnalytics = new Map();
    analyticsSnapshot.docs.forEach((doc) => {
        ...
        const totals = data.totals && typeof data.totals === "object"
            ? data.totals
            : {};
        const existing = aggregatedAnalytics.get(firestoreProductId) ??
            {
                firestoreProductId,
                clicks30d: 0,
                impressions30d: 0,
        ...
        existing.impressions30d += Math.max(0, toFiniteNumber(totals.impressions) ?? 0);
        existing.views30d += Math.max(0, toFiniteNumber(totals.views) ?? 0);
        const ctr = toFiniteNumber(totals.ctr);
        if (ctr !== null) {
            existing.ctr30d = ctr;
        }
        existing.groupingHints = Array.from(new Set([
    ...
    const productMap = await fetchProductsById(analyticsByProductId.map((item) => item.firestoreProductId));
    ...
    analyticsByProductId.forEach((analytics) => {
        ...
        rankingCandidates.push({
            ...
            cachedSubscriptionPlan: normalizeText(cachedMetafields.subscription_plan) || null,
    ...
    const shopifyInputs = await fetchShopifyRankingInputs(Array.from(new Set(rankingCandidates.map((candidate) => candidate.shopifyProductId))));
    ...
    const maxClicksByGroup = new Map();
    ...
    const rankingEntries = eligibleCandidates
        .map((candidate) => {
        const shopifyInput = shopifyInputs.get(candidate.shopifyProductId);
        const subscriptionPlan = shopifyInput?.subscriptionPlan ??
            candidate.cachedSubscriptionPlan ??
            null;
        const avgRating = shopifyInput?.avgRating ??
            candidate.cachedAvgRating ??
            0;
        const reviewCount = shopifyInput?.reviewCount ??
            candidate.cachedReviewCount ??
            0;
        const groupKey = resolveGroupingKey(candidate, shopifyInput);
        const maxClicksInGroup = maxClicksByGroup.get(groupKey) ?? 0;
        const clickScore = maxClicksInGroup > 0
            ? Math.min(24, (candidate.clicks30d / maxClicksInGroup) * 24)
            : 0;
        const reviewScore = (clamp(avgRating, 0, 5) / 5) * 28 +
            Math.min(12, (Math.max(reviewCount, 0) / 50) * 12);
        const penalty = avgRating < 3 ? 12 : avgRating < 3.5 ? 6 : 0;
        const calculatedRankScore = getPlanScore(subscriptionPlan) +
            clickScore +
            reviewScore -
            penalty;
        const shouldUsePlanFallback = isEffectivelyZero(candidate.clicks30d) &&
    ...
    await mapWithConcurrency(rankingEntries, SHOPIFY_UPDATE_CONCURRENCY, async (entry) => {
        try {
            await (0, shopifyMetafields_1.setCustomProductMetafields)({
            ...
            failedProducts += 1;
            console.error(`[product-ranking-sync] Failed to update Shopify product ${entry.shopifyProductId} (Firestore product ${entry.firestoreProductId})`, error);
        }
    });
    return {
        startedAt,
...
exports.syncProductRankingToShopify = syncProductRankingToShopify;
