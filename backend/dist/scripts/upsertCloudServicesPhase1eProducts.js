...
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-cloud-services-phase1e-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "cloud-services";
const PREVIEW_FILE_PREFIX = "cloud-services-phase1e-preview-";
...
const writePreflightReport = async (summary) => {
    ...
    const reportPath = path.join(EXPORTS_DIR, `cloud-services-phase1e-upload-preflight-${timestamp}.json`);
...
const upsertShopifyProduct = async (row) => {
    ...
    const existingVariant = Array.isArray(existingProduct?.variants)
        ...
        : null;
    const primaryType = row.customTypeMultiple[0] ?? row.collectionTitles[0] ?? "Cloud Services";
    const payload = {
        product: {
...
const applyDataset = async (rows) => {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const reportPath = path.join(EXPORTS_DIR, `cloud-services-phase1e-upload-report-${timestamp}.json`);
...
main().catch((error) => {
    console.error("Cloud services phase 1e upsert failed:", error);
    process.exitCode = 1;
});
