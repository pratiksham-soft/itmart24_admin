...
const BATCH_LABEL = "software-batch33";
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-software-batch33-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "software";
const PREVIEW_FILE_PREFIX = "software-batch33-preview-";
    ...
    rows.forEach((row, index) => {
        ...
        row.collectionHandles.forEach((handle, collectionIndex) => {
            ...
            if (!TARGET_COLLECTION_HANDLES.has(handle)) {
                validationErrors.push(`${rowLabel} collection handle outside batch 33 scope: ${handle}`);
            }
...
const resolveLogoSourceUrl = async (row) => {
    const directExtension = normalizeLogoExtension(path.extname(new URL(row.logoSourceUrl).pathname));
...
const downloadLogoAsset = async (row) => {
    ...
    if (!extension) {
        throw new Error(`Unsupported logo format for ${row.vendor}: ${contentType || "unknown"}`);
    }
...
main().catch((error) => {
    console.error("Software batch 33 upsert failed:", error);
    process.exitCode = 1;
});
