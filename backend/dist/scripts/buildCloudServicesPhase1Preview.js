...
const wordCount = (value) => stripHtml(value)
    .split(/\s+/)
...
const buildSeoTitle = (spec) => `${spec.title} | ${spec.vendor} ${spec.collections[0]}`;
const buildSeoDescription = (spec) => {
...
const buildFeaturesText = (spec) => spec.featureList.map((item) => `- ${item}`).join("\n");
const buildPlansPricingText = (spec) => spec.pricingBullets.map((item) => `- ${item}`).join("\n");
const buildProsConsText = (spec) => {
...
const latestZeroCollectionsCsvPath = () => {
    const files = fs_1.default
        ...
        .filter((fileName) => fileName.startsWith("zero-product-collections-") &&
        ...
        .map((fileName) => ({
        fileName,
        fullPath: path_1.default.join(EXPORTS_DIR, fileName),
        mtimeMs: fs_1.default.statSync(path_1.default.join(EXPORTS_DIR, fileName)).mtimeMs,
    }))
...
const PRODUCT_SPECS = [
    {
        ...
        summary: "The Business Starter tier focuses on professional email, lightweight collaboration, and entry-level administration for organizations that want Google-hosted productivity tools with a custom domain.",
        bestFor: "It is best for small teams, founders, and service businesses that need branded email, shared calendar access, and collaboration tools without moving straight to a larger suite.",
        fitNarrative: "It is a strong match for Business Email Hosting and Google Workspace Hosting because the plan is the base commercial edition with custom business email, pooled storage, and Google's standard management controls.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "Business Standard expands the Google Workspace stack with larger pooled storage, richer meeting features, and stronger collaboration tooling while keeping the same custom email foundation.",
        bestFor: "It is best for growing teams that need business email hosting plus more storage, appointment scheduling, recording, and broader collaboration support.",
        fitNarrative: "It fits Google Workspace Hosting, Business Email Hosting, and Enterprise Email Solutions because it pairs domain email with a deeper productivity bundle and higher operating limits than the starter tier.",
        featureList: [
    ...
    },
    {
        ...
        officialUrl: "https://www.microsoft.com/en-us/microsoft-365/business/microsoft-365-plans-and-pricing",
        sourceUrls: [
        ...
        pricingModel: "Subscription",
        billingCycle: "Annual",
        summary: "Microsoft 365 Business Basic is the lower-cost commercial Microsoft email and collaboration plan for teams that want Exchange-backed mailboxes, Teams meetings, and OneDrive storage without desktop Office apps.",
        bestFor: "It is best for small businesses that need business-grade email, calendaring, cloud storage, and Microsoft collaboration without paying for the full desktop suite.",
        fitNarrative: "It is a direct fit for Business Email Hosting and Microsoft 365 Hosting because custom email, Exchange, Teams, and OneDrive are the core reasons buyers shop in those collections.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Annual",
        summary: "Microsoft 365 Business Standard adds desktop Office apps and a broader productivity footprint on top of the same Exchange, Teams, and storage foundation used by the lower tier.",
        bestFor: "It is best for established teams that need business email plus full desktop Office applications, webinar hosting, and standard Microsoft collaboration workflows.",
        fitNarrative: "It fits Microsoft 365 Hosting and Enterprise Email Solutions because it combines branded email with the fuller Microsoft business suite that many mid-market buyers standardize on.",
        featureList: [
            "Custom business email",
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "DNS-5 is DNS Made Easy's entry paid managed DNS plan for smaller production workloads that still need SLA-backed reliability, failover support, and low-latency authoritative DNS.",
        bestFor: "It is best for small businesses, agencies, and operations teams that need managed DNS without buying a larger multi-domain package on day one.",
        fitNarrative: "It maps cleanly to DNS Management and Premium DNS because it is an authoritative managed DNS subscription with SLA commitments, analytics, and failover instead of a free registrar add-on.",
        featureList: [
            "5 domains included",
            "1,500 DNS records",
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "DNS-25 is the larger managed DNS membership for teams that need more domains, higher query limits, analytics, and global traffic control on a published commercial plan.",
        bestFor: "It is best for agencies, DevOps teams, and growing infrastructure operators that need higher-capacity managed DNS than an entry plan can provide.",
        fitNarrative: "It is a direct match for Premium DNS and DNS Management because the plan adds traffic control, analytics, and larger usage ceilings while staying in the same authoritative DNS platform.",
        featureList: [
            "25 domains included",
            "7,500 DNS records",
    ...
    },
    {
        ...
        summary: "DNSimple Solo is a pay-as-you-go DNS and domain management plan built around hosted zones, DNS query volume, and lightweight control-plane automation instead of a fixed platform bundle.",
        bestFor: "It is best for individuals, developers, and small operations teams that want authoritative DNS, DNSSEC, and domain management without a base subscription fee.",
        fitNarrative: "It fits DNS Management and Premium DNS because the service is centered on hosted zones, Anycast DNS, DNSSEC, domain control, and operational automation rather than simple registrar defaults.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Usage-based",
        billingCycle: "Monthly",
        summary: "DNSimple Teams adds a base subscription with team features such as access control, SSO support, broader activity history, and an SLA while retaining the same zone-based DNS billing model.",
        bestFor: "It is best for agencies, collaborative infrastructure teams, and businesses that need managed DNS governance rather than a single-user setup.",
        fitNarrative: "It fits Premium DNS and DNS Management because the plan keeps Anycast DNS and zone-based control while adding multi-user administration, SLA coverage, and stronger team operations.",
        featureList: [
    ...
    },
    {
        ...
        summary: "Namecheap's domain transfer offer is a registrar transfer product with published transfer pricing, included privacy on eligible names, and a one-year renewal extension for supported TLDs.",
        bestFor: "It is best for domain owners who want lower transfer pricing, simpler portfolio consolidation, and registrar-side privacy coverage on common TLDs.",
        fitNarrative: "It belongs in Domain Transfer because the product is a registrar transfer workflow rather than a fresh domain registration listing or a DNS hosting subscription.",
        featureList: [
    ...
    },
    {
        ...
        startingPrice: 12.99,
        pricingModel: "One-time purchase",
        summary: "GoDaddy Domain Transfer is a registrar transfer offer for moving an existing domain into GoDaddy with a flat .com transfer price, bundled privacy, and a free year of registration on supported transfers.",
        bestFor: "It is best for buyers who want a mainstream registrar, a simple transfer flow, and centralized domain management in the same account.",
        fitNarrative: "It fits Domain Transfer because the page is built specifically around moving an existing domain to a new registrar rather than selling first-time registrations or DNS-only services.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Annual",
        summary: "Sucuri Basic is the entry full-platform website security plan that combines monitoring, malware cleanup, blacklist support, a website firewall, and DDoS mitigation in one annual subscription.",
        bestFor: "It is best for small business websites and agency-managed properties that need a bundled security response product rather than standalone scanning or a single point tool.",
        fitNarrative: "It fits Website Security, Malware Removal, Web Application Firewall, and DDoS Protection because the plan explicitly combines malware cleanup, blacklist monitoring, firewall coverage, and advanced DDoS mitigation.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Annual",
        summary: "Sucuri Professional is the higher security platform tier with faster cleanup response and the same broad website protection stack used for firewalling, monitoring, cleanup, and blacklist handling.",
        bestFor: "It is best for commercial websites that want the Sucuri platform with quicker incident turnaround than the entry plan.",
        fitNarrative: "It is a direct fit for Website Security, Malware Removal, Web Application Firewall, and DDoS Protection because the official plan table keeps all of those protection layers in scope and upgrades response speed.",
        featureList: [
            "Continuous security scanning",
            "Malware and hack removal",
            "Blacklist monitoring",
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "SiteLock Basic is the lower-priced monthly website security package focused on daily scanning, automatic malware removal, backup, and baseline monitoring for smaller sites.",
        bestFor: "It is best for budget-conscious site owners who want continuous scanning and cleanup in a simpler monthly package.",
        fitNarrative: "It maps to Website Security and Malware Removal because the plan centers on daily scanning, automatic malware removal, and backup rather than acting purely as an SSL or registrar add-on.",
        featureList: [
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "SiteLock Pro is the higher monthly website security plan for sites that need the SiteLock stack plus its comprehensive CDN and web application firewall layer.",
        bestFor: "It is best for business sites with more traffic or stronger perimeter protection needs than the basic monitoring package provides.",
        fitNarrative: "It belongs in Website Security, Malware Removal, and Web Application Firewall because SiteLock explicitly adds a comprehensive CDN and WAF on top of scanning, cleanup, and backup.",
        featureList: [
            "Daily code, database, and CMS scanning",
    ...
    },
    {
        ...
        pricingModel: "Subscription",
        billingCycle: "Monthly",
        summary: "DigiCert Secure Site is a premium TLS/SSL subscription that combines certificate coverage with reputation tooling, malware checks, and blocklist checks for organizations that want more than a bare certificate issue workflow.",
        bestFor: "It is best for businesses and larger web properties that want a premium certificate offering with added validation and visibility tools.",
        fitNarrative: "It belongs in SSL Certificates because the core product is a DigiCert TLS/SSL subscription, even though the bundle also includes security-related supporting tools.",
        featureList: [
    ...
    },
    {
        ...
        startingPrice: 88,
        pricingModel: "Subscription",
        summary: "Sectigo Single Domain DV SSL is a lower-complexity domain-validation certificate for websites that need standard encrypted HTTPS with fast issuance and a lower barrier than organization-validated products.",
        bestFor: "It is best for blogs, brochure sites, smaller business sites, and teams that want commercial certificate support without paying for premium enterprise validation tiers.",
        fitNarrative: "It is a direct fit for SSL Certificates because the offer is a domain validation certificate product with publicly stated certificate warranty, quick issuance, and commercial support.",
        featureList: [
...
const buildPreviewRows = async () => {
    ...
    const cloudCollectionByTitle = new Map(categoryRows
        .filter((row) => row.top_category === "Cloud Services")
        .map((row) => [row.final_category, row.collection_handle]));
    const filterDefinitions = new Map(filterRows
        .filter((row) => row.profile_id === "cloud-services" &&
    ...
    const zeroCloudCollections = zeroCollectionRows.filter((row) => row.top_category === "Cloud Services" &&
        Number(row.product_count || 0) === 0);
    const previewRows = PRODUCT_SPECS.map((spec) => {
        ...
        return {
            ...
            collectionHandles: spec.collections.map((collection) => cloudCollectionByTitle.get(collection)),
    ...
    const coverageByCollection = zeroCloudCollections.map((collection) => {
        const count = previewRows.filter((row) => row.customTypeMultiple.includes(collection.final_category)).length;
        return {
    ...
    return {
        ...
        coveredCollections: coverageByCollection.filter((item) => item.productsAssigned >= 2),
        shortfilledCollections: coverageByCollection.filter((item) => item.productsAssigned > 0 && item.productsAssigned < 2),
        uncoveredCollections: coverageByCollection.filter((item) => item.productsAssigned === 0),
        blockedCollections: coverageByCollection.filter((item) => !item.liveCollectionFound),
        zeroCollectionsPath,
    };
};
...
const main = async () => {
    ...
    const { previewRows, coverageByCollection, coveredCollections, shortfilledCollections, uncoveredCollections, blockedCollections, zeroCloudCollections, zeroCollectionsPath, } = await buildPreviewRows();
    const jsonPath = path_1.default.join(EXPORTS_DIR, `cloud-services-phase1-preview-${timestamp}.json`);
    const csvPath = path_1.default.join(EXPORTS_DIR, `cloud-services-phase1-preview-${timestamp}.csv`);
    const reportPath = path_1.default.join(EXPORTS_DIR, `cloud-services-phase1-validation-${timestamp}.json`);
    const validation = {
    ...
    await Promise.all([
        ...
        fs_1.default.promises.writeFile(csvPath, buildPreviewCsv(previewRows), "utf8"),
