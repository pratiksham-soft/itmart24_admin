...
const buildProviderPlans = (vendor, officialPath, seeds) => seeds.map((seed) => ({
    code: seed.code,
...
const buildDescriptionHtml = (provider, plan) => {
    const paragraphs = [
        ...provider.descriptionLead.map(toSentence),
        toSentence(`${plan.title} is the ${provider.vendor} ${plan.planName} plan on the provider's official ${provider.officialSourceLabel} page.`),
        toSentence(`The current lowest publicly visible starting price on the official page is ${toPriceString(plan.lowestPrice)} per month-equivalent, and the pricing details are captured in the plans and pricing metafield for review.`),
        toSentence(`Key published features for this plan include ${plan.features
            .slice(0, 5)
            .join(", ")
            .replace(/, ([^,]*)$/, ", and $1")}.`),
        ...provider.descriptionSupport.map(toSentence),
        toSentence(`This product is suitable for shoppers who want ${provider.platformLabel} with a public feature list, a visible starting price, and a plan structure that is easy to compare against other shared-hosting offers.`),
        toSentence(`It is not represented here as VPS, dedicated, reseller, or Windows hosting, because the dataset is intentionally limited to offers that fit the Linux Shared Hosting collection based on the official provider materials reviewed for this task.`),
        toSentence(`Where pricing changes by billing term or renews at a different rate, those facts are preserved in the pricing notes rather than hidden inside the description.`),
    ].filter(Boolean);
    const featureItems = plan.features.map((feature) => `<li>${escapeHtml(feature)}</li>`);
    const pricingItems = plan.pricingSummary.map((item) => `<li>${escapeHtml(item)}</li>`);
    return [
...
const normalizeFilterValues = (filterDefinitions, filterValues) => {
    ...
    filterDefinitions.forEach((definition) => {
        ...
        if (invalid.length > 0) {
            throw new Error(`Invalid values for ${definition.key}: ${invalid.join(", ")}`);
        }
...
const writePreviewArtifacts = async (rows) => {
    ...
    const jsonPath = path.join(EXPORTS_DIR, `linux-shared-hosting-preview-${timestamp}.json`);
    const csvPath = path.join(EXPORTS_DIR, `linux-shared-hosting-preview-${timestamp}.csv`);
    const reportPath = path.join(EXPORTS_DIR, `linux-shared-hosting-validation-${timestamp}.json`);
    const validation = {
...
const applyDataset = async (rows) => {
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const reportPath = path.join(EXPORTS_DIR, `linux-shared-hosting-upload-report-${timestamp}.json`);
