...
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-domain-registration-logos");
...
const loadTargetCategory = async () => {
    ...
    const match = rows.find((row) => row.top_category === "Cloud Services" &&
        row.subcategory === TARGET_SUBCATEGORY &&
        row.final_category === TARGET_FINAL_CATEGORY &&
        row.collection_handle === TARGET_COLLECTION_HANDLE);
    if (!match) {
        throw new Error(`Could not find ${TARGET_FINAL_CATEGORY} in ${CATEGORY_CSV_PATH}`);
    }
    return match;
};
const buildBulletsText = (items) => items.map((item) => `- ${item}`).join("\n");
const stripTrailingPunctuation = (value) => value.trim().replace(/[.:\s]+$/g, "");
const toTitleSentence = (value) => {
...
const buildProfessionalPros = (provider) => provider.features
    .slice(0, 2)
    .map((feature) => toTitleSentence(feature));
const buildProfessionalCons = (provider) => provider.factualCons.map((item) => {
    const replacements = [
        [
...
const buildDescriptionHtml = (provider) => {
    ...
    const paragraphs = [
        toSentence(`${provider.brand} Domain Registration is designed for individuals, startups, and businesses that want to secure and manage domain names through a single registrar account.`),
        toSentence(`The service supports the core tasks buyers expect from a registrar, including domain search, registration, renewal, transfers, DNS or account management, and extension selection across a broader catalog of TLDs.`),
        toSentence(`Current entry pricing starts at $${toPriceString(provider.lowestPrice)}, with exact costs varying by extension, promotional eligibility, and renewal terms.`),
        toSentence(`${provider.brand} includes key capabilities such as ${featureExcerpt}, making it suitable for buyers who want a balance of registration convenience, domain control, and ongoing account management.`),
        toSentence(`This registrar is a practical fit for projects that need a clean path from domain purchase to ownership management, especially when privacy, transfer handling, or support availability are important buying factors.`),
        toSentence(`For most buyers, the main decision points are first-year pricing, renewal pricing, transfer costs, privacy availability, domain-management tools, and how clearly the registrar presents those terms before checkout.`),
        toSentence(`A strong domain registration service should make it easy to register a name, renew it on time, transfer it when needed, manage DNS settings, and keep ownership details and account access under control over the full life of the domain.`),
        toSentence(`${provider.brand} is most useful when you want a registrar that can support both the initial purchase and the longer-term operational side of domain ownership without adding unnecessary complexity.`),
        toSentence(`Notable strengths include ${strengths}, while buyers should still review renewal pricing, extension-specific policies, and offer conditions before checkout.`),
        toSentence(`Important considerations include ${considerations || "variation in pricing and terms between different extensions and offers"}.`),
        toSentence(`Because registration terms can vary between standard TLDs, country-code domains, transfers, and promotional offers, it is best to confirm the exact extension-specific price and renewal policy that matches your intended domain before placing an order.`),
    ].filter(Boolean);
    const featureItems = provider.features.map((feature) => `<li>${escapeHtml(feature)}</li>`);
    const pricingItems = provider.pricingSummary.map((item) => `<li>${escapeHtml(item)}</li>`);
    const prosConsItems = [
...
const buildSeoDescription = (provider) => {
    const description = `${provider.brand} domain registration with ${provider.features
        .slice(0, 2)
        .join(", ")
        .toLowerCase()}, and official pricing from $${toPriceString(provider.lowestPrice)}.`;
    return description.length <= 160
        ? description
        : description.slice(0, 157).trimEnd() + "...";
};
const normalizeFilterValues = (filterDefinitions, filterValues) => {
    ...
    Object.entries(filterValues).forEach(([key, values]) => {
        ...
        const invalid = requestedValues.filter((value) => !definition.allowedValues.includes(value));
        if (invalid.length > 0) {
...
const writePreviewArtifacts = async (rows) => {
    await ensureDir(EXPORTS_DIR);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const jsonPath = path.join(EXPORTS_DIR, `domain-registration-preview-${timestamp}.json`);
    const csvPath = path.join(EXPORTS_DIR, `domain-registration-preview-${timestamp}.csv`);
    const reportPath = path.join(EXPORTS_DIR, `domain-registration-validation-${timestamp}.json`);
    ...
    const validation = {
        ...
        invalidPriceRows: rows
            ...
            .map((row) => row.handle),
        invalidUrlRows: rows
            .filter((row) => !isValidUrl(row.sourceUrl) ||
...
const buildMarketplaceFilterReferenceMap = async (filterKeys) => {
    ...
    for (const key of filterKeys) {
      ...
      `,
        ...
        const metaobjectType = metaobjectDefinitionResponse.data?.data?.metaobjectDefinition?.type ??
            null;
        if (!metaobjectType) {
            continue;
        }
...
const resolveLogoSourceUrl = async (provider) => {
    if (provider.logoUrlHint) {
        return provider.logoUrlHint;
    }
    const response = await axios_1.default.get(provider.logoPageUrl, {
        timeout: 30000,
        responseType: "text",
        headers: {
    ...
    for (const candidate of candidates) {
        try {
            ...
            if (Number(headResponse.status) >= 200 &&
...
const downloadLogoAsset = async (provider) => {
...
const applyDataset = async (rows) => {
    ...
    const duplicateHandles = rows.filter((row) => {
        const matches = existingProducts.filter((product) => product.handle === row.handle);
    ...
    for (const row of rows) {
        ...
        try {
            let logo = providerLogoCache.get(provider.vendor) ?? null;
            let logoError = null;
            if (!logo) {
                try {
            ...
            const imageAction = logo?.fileUrl
                ? await upsertProductImage(productResult.productId, logo.fileUrl, row.imageAltText)
                : "skipped";
            results.push({
    ...
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const reportPath = path.join(EXPORTS_DIR, `domain-registration-upload-report-${timestamp}.json`);
