...
const CATEGORY_COLLECTIONS_PATH = path.resolve(__dirname, "../../imports/category-collections.csv");
const CATEGORIES_PATH = path.resolve(__dirname, "../../imports/categories.csv");
const FILTERS_CSV_PATH = path.resolve(__dirname, "../../doc/shopify-filter-definitions.csv");
const slugifyPathValue = (value) => value
    .toLowerCase()
    .replace(/&/g, " and ")
    .replace(/[^a-z0-9]+/g, "-")
    .replace(/^-+|-+$/g, "")
...
const SOFTWARE_PRODUCTS_BASE_ROOT = path.resolve(__dirname, "../../exports/software-products");
const INPUT_ROOT_ARG = getCliArgValue("--root") || getCliArgValue("--input-root");
const SOFTWARE_PRODUCTS_ROOT = INPUT_ROOT_ARG
    ? path.resolve(process.cwd(), INPUT_ROOT_ARG)
    : SOFTWARE_PRODUCTS_BASE_ROOT;
const EXISTING_EXPORT_CSV_PATH = path.resolve(SOFTWARE_PRODUCTS_BASE_ROOT, "shopify-products-category-report.csv");
const EXPORTS_DIR = path.resolve(__dirname, "../../exports");
const LOGO_TEMP_ROOT = path.resolve(EXPORTS_DIR, "tmp-software-products-import-logos");
const IMPORT_SCOPE_SEGMENT = path.relative(SOFTWARE_PRODUCTS_BASE_ROOT, SOFTWARE_PRODUCTS_ROOT) ||
...
const CATEGORY_MAPPING_ALIASES = new Map([
...
const normalizeRowKey = (value) => String(value ?? "")
    .trim()
    .toLowerCase()
    .replace(/[^a-z0-9]+/g, "");
...
const getRowHandle = (row) => getRowValue(row, "Handle", "handle");
const getRowTitle = (row) => getRowValue(row, "Title", "title");
const getRowVendor = (row) => getRowValue(row, "Vendor", "vendor");
const getRowBodyHtml = (row) => getRowValue(row, "Body (HTML)", "body_html");
const getRowVariantPrice = (row) => getRowValue(row, "Variant Price", "variant_price");
const getRowSeoTitle = (row) => getRowValue(row, "SEO Title", "seo_title");
const getRowSeoDescription = (row) => getRowValue(row, "SEO Description", "seo_description");
const getRowStatus = (row) => getRowValue(row, "Status", "status");
const getRowOfficialUrl = (row) => getRowValue(row, "product.metafields.custom.custom", "Product Metafields Custom Custom");
const getRowLogoImage = (row) => getRowValue(row, "product.metafields.custom.logo_image", "Product Metafields Custom Logo Image");
const getRowTargetUseCase = (row) => getRowValue(row, "product.metafields.custom.target_use_case", "Product Metafields Custom Target Use Case");
const getRowPlansPricing = (row) => getRowValue(row, "product.metafields.custom.plans_pricing", "Product Metafields Custom Plans Pricing");
const getRowProductFeatures = (row) => getRowValue(row, "product.metafields.custom.product_features", "Product Metafields Custom Product Features");
const normalizeCsvRow = (row) => Object.fromEntries(Object.entries(row).map(([key, value]) => [
    key.replace(/^\uFEFF/, "").replace(/^"|"$/g, ""),
    typeof value === "string" ? value.trim() : String(value ?? ""),
]));
const isEmptyCsvRow = (row) => Object.keys(row).length === 0 ||
    Object.values(row).every((value) => !String(value ?? "").trim());
const shouldUseDefaultProductHeaders = (filePath, fileText) => {
    ...
    const firstNonEmptyLine = fileText
        .split(/\r?\n/)
        .map((line) => line.trim())
        .find(Boolean) ?? "";
    ...
    return !(normalizedFirstLine.startsWith("handle,") ||
...
const slugify = (value) => value
    ...
    .replace(/^-+|-+$/g, "")
    .replace(/-{2,}/g, "-");
const normalizeText = (value) => value.toLowerCase().replace(/[^a-z0-9]+/g, " ").trim().replace(/\s+/g, " ");
const normalizeHandle = (value) => slugify(value);
const normalizeComparableTitle = (value) => normalizeText(value)
    .replace(/\bb2b\b/g, " ")
    .replace(/\bedition\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
const normalizeComparableVendor = (value) => normalizeText(value)
    .replace(/\b(inc|incorporated|llc|ltd|limited|holdings|corp|corporation|co)\b/g, " ")
    .replace(/\s+/g, " ")
    .trim();
const countWords = (value) => stripHtml(value)
    .split(/\s+/)
...
const parseAnyNumericPrice = (value) => {
...
const formatNumericPrice = (value) => {
...
const isRetryableShopifyError = (error) => {
...
const withShopifyRetries = async (label, fn, maxAttempts = 5) => {
    let lastError;
...
const formatShopifyError = (error) => {
...
const isHandleAlreadyTakenError = (error) => formatShopifyError(error).toLowerCase().includes("has already been taken");
...
const resolveOfficialUrl = (row, overrideOfficialUrl) => {
...
const splitAllowedValues = (value) => value
    ...
    .map((item) => item.trim())
    .filter(Boolean);
const normalizeMultilineSourceText = (value) => String(value ?? "")
    .replace(/\\r\\n/g, "\n")
    .replace(/\\n/g, "\n")
    .replace(/\\r/g, "\n")
    .replace(/\r\n/g, "\n")
    .replace(/\r/g, "\n");
const extractCleanLines = (value) => normalizeMultilineSourceText(value)
    .split("\n")
    .map((line) => line
    .trim()
    .replace(/^[-*.\u2022]+\s*/, "")
...
const normalizeLogoExtension = (extension) => extension.toLowerCase() === ".jpg" ? ".jpeg" : extension.toLowerCase();
...
const buildSummaryCounts = (totalRows, logRows) => ({
    totalRows,
    imported: logRows.filter((row) => row.status === "imported").length,
    skipped_existing_shopify: logRows.filter((row) => row.status === "skipped_existing_shopify").length,
    skipped_existing_current_job: logRows.filter((row) => row.status === "skipped_existing_current_job").length,
    updated_existing_type_multiple: logRows.filter((row) => row.status === "updated_existing_type_multiple").length,
    skipped_missing_required_data: logRows.filter((row) => row.status === "skipped_missing_required_data").length,
    skipped_pricing_unavailable: logRows.filter((row) => row.status === "skipped_pricing_unavailable").length,
...
const writeJsonReport = async (sourceRowsCount, logRows, mode, skippedFolders) => {
    await ensureDir(EXPORTS_DIR);
    const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
    const summary = buildSummaryCounts(sourceRowsCount, logRows);
    const reportPath = path.join(EXPORTS_DIR, `${REPORT_PREFIX}-${mode}-${timestamp}.json`);
...
const writeAttentionCsvReport = async (logRows, timestamp) => {
    const rows = logRows.filter((row) => row.shopifyProductId !== null &&
        (row.price === "0" || row.logoStatus === "missing"));
    const reportPath = path.join(EXPORTS_DIR, `${ATTENTION_REPORT_PREFIX}-${timestamp}.csv`);
    const header = [
        "Product Name",
        "Product ID",
        "Pricing",
...
const resolveCategoryMapping = async (folderName, sourceCsvPath, softwareCategoryRows) => {
    ...
    const sourceTypes = dedupe(sourceRows
        .map((row) => getRowValue(row, "Type", "type"))
        .filter(Boolean));
    const aliasValues = dedupe([folderName, csvStem, ...sourceTypes]
    ...
    const mappingRow = softwareCategoryRows.find((row) => candidateHandles.includes(normalizeHandle(String(row.collection_handle ?? "")))) ??
        softwareCategoryRows.find((row) => candidateNames.includes(normalizeText(String(row.final_category ?? "")))) ??
        softwareCategoryRows.find((row) => candidateNames.includes(normalizeText(String(row.collection_title ?? ""))));
    if (!mappingRow) {
        throw new Error(`Could not resolve category mapping for folder "${folderName}" (${sourceCsvPath})`);
    }
    const categories = await readCsv(CATEGORIES_PATH);
    const categoryExists = categories.some((row) => normalizeText(row.mainCategory ?? row.Category ?? "") ===
        normalizeText("Software") &&
        normalizeText(row.subCategory ?? row.Subcategory ?? "") ===
            normalizeText(String(mappingRow.final_category ?? "")));
    if (!categoryExists) {
        throw new Error(`Category mapping for ${mappingRow.final_category} is missing in ${CATEGORIES_PATH}`);
    }
    return {
...
const discoverImportTargets = async () => {
    ...
    for (const dirent of dirents
        ...
        const preferredCsv = csvFiles.find((fileName) => normalizeText(path.basename(fileName, path.extname(fileName)).replace(/_/g, " ")) ===
            normalizeText(dirent.name)) ?? csvFiles[0];
        const sourceCsvPath = path.join(folderPath, preferredCsv);
        let categoryMapping;
        try {
            categoryMapping = await resolveCategoryMapping(dirent.name, sourceCsvPath, softwareCategoryRows);
        }
        catch (error) {
            skippedFolders.push({
                folderName: dirent.name,
...
const loadExistingExport = async () => {
    const rows = (await readCsv(EXISTING_EXPORT_CSV_PATH));
    const byHandle = new Map();
    const byCollectionHandle = new Map();
    rows.forEach((row) => {
...
const findExistingMatch = (row, existing, collectionHandle) => {
    ...
    const titleMatches = collectionRows.filter((candidate) => normalizeComparableTitle(candidate.title ?? "") === sourceTitle);
    if (titleMatches.length === 1) {
        return titleMatches[0];
    }
    return (titleMatches.find((candidate) => {
        const candidateVendor = normalizeComparableVendor(candidate.vendor ?? "");
        return (candidateVendor === comparableSourceVendor ||
...
const buildCurrentJobKeys = (row) => {
    ...
    const titleUrlKey = titleKey && officialUrl ? `${titleKey}::${cleanOfficialUrl(officialUrl)}` : "";
    return {
        handleKey,
        titleKey,
        titleUrlKey,
...
const parseTypeMultipleValues = (value) => {
    const trimmed = String(value ?? "").trim();
    if (!trimmed) {
        return [];
    }
    ...
    return dedupe(normalizeMultilineSourceText(trimmed)
        .split("\n")
        .map((item) => item.trim())
        .filter(Boolean));
};
const inferPricingModel = (pricingLines, numericPrice) => {
...
const inferPriceBand = (numericPrice) => {
...
const inferDeploymentModel = (bodyText, featureLines) => {
...
const inferCollaborationMode = (bodyText, featureLines) => {
    const text = normalizeText([bodyText, ...featureLines].join(" "));
    const values = [];
...
const inferDeveloperFeatures = (bodyText, featureLines) => {
    const text = normalizeText([bodyText, ...featureLines].join(" "));
    const values = [];
    if (/\bapi\b/.test(text)) {
...
const buildFilterValues = (price, bodyText, featureLines, pricingLines, definitions) => {
    ...
    return Object.fromEntries(Object.entries(candidateValues)
        .map(([key, values]) => {
...
const buildProductFeatures = (featureLines) => featureLines.map((line) => `- ${toSentence(line)}`).join(MULTILINE_SEPARATOR);
const buildPlansPricingWithFallback = (pricingLines, usedZeroPriceFallback) => {
    const lines = [...pricingLines];
    if (usedZeroPriceFallback) {
        lines.push('To visit product official website click "Get Now".');
    }
    return lines
        .map((line) => line === 'To visit product official website click "Get Now".'
        ? line
        : `- ${toSentence(line)}`)
        .join(MULTILINE_SEPARATOR);
};
const buildProsCons = (featureLines, pricingLines, numericPrice, usedZeroPriceFallback) => {
    const pricingText = pricingLines.join(" ");
    const planPrices = dedupe(Array.from(pricingText.matchAll(/\$?\s*(\d[\d,]*(?:\.\d+)?)/g))
    ...
    if (usedZeroPriceFallback) {
        ...
        cons.push(`Con: Pricing varies by plan, with higher visible tiers up to ${planPrices[planPrices.length - 1]}.`);
    }
    else {
        cons.push("Con: Feature availability should be checked against the specific plan before purchase.");
    }
    cons.push("Con: Buyers should confirm integration scope, account structure, and rollout effort against their operational needs.");
    return [...pros, ...cons]
        .map((line) => `- ${line}`)
        .join(MULTILINE_SEPARATOR);
};
const buildBodyHtml = (title, vendor, collectionTitle, bodyText, featureLines, pricingLines, numericPrice, filterValues, usedZeroPriceFallback) => {
    ...
    const deploymentSentence = filterValues.deployment_model?.length > 0
    ...
    const collaborationSentence = filterValues.collaboration_mode?.length > 0
    ...
    const developerSentence = filterValues.developer_features?.length > 0
...
const loadLocalLogoFiles = async (logoDirs) => {
...
const findBestLocalLogo = async (row, logoDirs) => {
    const files = await loadLocalLogoFiles(logoDirs);
    const officialUrl = cleanOfficialUrl(getRowOfficialUrl(row));
    const hostToken = (() => {
        try {
    ...
    for (const candidate of candidates) {
        const partial = files.find((file) => file.stem.includes(candidate) || candidate.includes(file.stem));
        if (partial) {
...
const convertRasterLogoToWebp120 = async (inputPath, outputPath) => {
...
const prepareLocalLogoAsset = async (localPath) => {
...
const resolveRemoteLogoSourceUrl = async (officialUrl) => {
    const directExtension = normalizeLogoExtension(path.extname(new URL(officialUrl).pathname));
    if (DIRECT_LOGO_SOURCE_EXTENSIONS.has(directExtension)) {
        return officialUrl;
    }
    const response = await axios_1.default.get(officialUrl, {
        timeout: 30000,
        responseType: "text",
        headers: {
    ...
    for (const candidate of candidates) {
        try {
            const logoResponse = await axios_1.default.get(candidate, {
                timeout: 30000,
...
const downloadRemoteLogoAsset = async (officialUrl, fileStem) => {
    await ensureDir(LOGO_TEMP_ROOT);
    ...
    const extensionFromUrl = normalizeLogoExtension(path.extname(new URL(sourceUrl).pathname));
...
const fetchPublicationIds = async () => {
    ...
    while (hasNextPage) {
        const response = await withShopifyRetries("fetch Shopify publications", () => shopifyGraphQL.post("", {
            query: `
...
const publishProduct = async (productId) => {
    ...
    const response = await withShopifyRetries(`publish product ${productId}`, () => shopifyGraphQL.post("", {
        query: `
...
const buildMarketplaceFilterReferenceMap = async (filterKeys) => {
    const { shopifyGraphQL } = await getShopifyClients();
    const definitionsResponse = await withShopifyRetries("load marketplace metafield definitions", () => shopifyGraphQL.post("", {
        query: `
...
const fetchProductByHandle = async (handle) => {
    ...
    const response = await withShopifyRetries(`fetch product by handle ${handle}`, () => shopifyRest.get("/products.json", {
        params: {
            handle,
            limit: 1,
        },
...
const fetchExistingProductStateById = async (productId) => {
    const { shopifyGraphQL } = await getShopifyClients();
    const response = await withShopifyRetries(`fetch existing product state ${productId}`, () => shopifyGraphQL.post("", {
        query: `
    ...
    return {
        ...
        customTypeMultiple: parseTypeMultipleValues(String(product?.typeMultiple?.value ?? "")),
    };
};
const updateTypeMultipleMetafield = async (productId, values) => {
    const { shopifyGraphQL } = await getShopifyClients();
    const response = await withShopifyRetries(`update type_multiple for ${productId}`, () => shopifyGraphQL.post("", {
        query: `
...
const resolveExistingProductAfterHandleConflict = async (product) => {
    for (let attempt = 1; attempt <= 4; attempt += 1) {
        ...
        if (liveExisting?.id) {
            ...
            const changed = existingState !== null &&
...
const createShopifyProduct = async (product) => {
    const { shopifyRest } = await getShopifyClients();
    const response = await withShopifyRetries(`create product ${product.handle}`, () => shopifyRest.post("/products.json", {
        product: {
            title: product.title,
...
const findExistingShopifyFileUrl = async (fileName) => {
    const { shopifyGraphQL } = await getShopifyClients();
    const response = await withShopifyRetries(`find Shopify file ${fileName}`, () => shopifyGraphQL.post("", {
        query: `
    ...
    for (const node of nodes) {
        const candidateUrl = typeof node?.image?.url === "string"
            ? node.image.url
...
const uploadFileToShopify = async (localPath, altText) => {
    ...
    const stagedUploadResponse = await withShopifyRetries(`create staged upload for ${fileName}`, () => shopifyGraphQL.post("", {
        query: `
    ...
    const fileCreateResponse = await withShopifyRetries(`create Shopify file ${fileName}`, () => shopifyGraphQL.post("", {
        query: `
    ...
    for (let attempt = 0; attempt < 20; attempt += 1) {
        const pollResponse = await withShopifyRetries(`poll Shopify file ${fileName}`, () => shopifyGraphQL.post("", {
            query: `
...
const setShopifyMetafields = async (productId, product, logoFileUrl, marketplaceFilterReferences) => {
    const { shopifyGraphQL } = await getShopifyClients();
    const inputs = [
        {
            namespace: "custom",
    ...
    const response = await withShopifyRetries(`set metafields for ${product.handle}`, () => shopifyGraphQL.post("", {
        query: `
...
const shouldUseZeroPriceFallback = (rawNumericPrice, pricingLines) => {
    ...
    const pricingText = normalizeText(pricingLines.join(" "));
    return (/\bnot publicly available\b/.test(pricingText) ||
...
const prepareProduct = async (row, target, filterDefinitions) => {
    const normalizedSourceHandle = normalizeHandle(getRowHandle(row));
    const override = (0, csCartB2BOverride_1.getProductImportOverride)(normalizedSourceHandle);
    ...
    const explicitPrice = typeof override?.price === "number"
        ? override.price
        : parsePositivePrice(getRowVariantPrice(row));
    const rawNumericPrice = parseAnyNumericPrice(getRowVariantPrice(row));
    const zeroPriceFallbackUsed = explicitPrice === null && shouldUseZeroPriceFallback(rawNumericPrice, pricingLines);
    ...
    if (missingFields.length > 0) {
        return {
            prepared: null,
            skipStatus: "skipped_missing_required_data",
    ...
    if (priceValue === null) {
        return {
            prepared: null,
            skipStatus: "skipped_pricing_unavailable",
            reason: "Pricing was blank or non-numeric, and no safe zero-pricing fallback was justified from the source data.",
        };
    }
    const filterValues = buildFilterValues(priceValue, sourceBodyText, featureLines, pricingLines, filterDefinitions);
    const productFeatures = buildProductFeatures(featureLines);
    const plansPricing = buildPlansPricingWithFallback(pricingLines, zeroPriceFallbackUsed);
    const prosCons = buildProsCons(featureLines, pricingLines, priceValue, zeroPriceFallbackUsed);
    const bodyHtml = buildBodyHtml(title, vendor, target.categoryMapping.collectionTitle, sourceBodyText, featureLines, pricingLines, priceValue, filterValues, zeroPriceFallbackUsed);
    const seoTitle = override?.seoTitle ??
        getRowSeoTitle(row) ??
        `${title} | ${vendor} ${target.categoryMapping.collectionTitle}`;
    const seoDescription = override?.seoDescription ??
        getRowSeoDescription(row) ??
...
const buildDryRunRows = async () => {
    ...
    const jobActions = [];
    const logRows = [];
    const handledHandleKeys = new Set();
    const handledTitleUrlKeys = new Set();
    const handledTitleKeys = new Set();
    let totalRows = 0;
    for (const target of targets) {
        ...
        for (const row of sourceRows) {
            ...
            const alreadyHandledInCurrentJob = (currentJobKeys.handleKey && handledHandleKeys.has(currentJobKeys.handleKey)) ||
            ...
            if (alreadyHandledInCurrentJob) {
                logRows.push({
                    ...
                    price: parseAnyNumericPrice(getRowVariantPrice(row)) === 0
                        ? "0"
            ...
            const existingMatch = findExistingMatch(row, existing, target.categoryMapping.collectionHandle);
            if (existingMatch) {
                jobActions.push({
                    kind: "existing",
                    categoryFolder: target.folderName,
...
const applyImport = async (jobActions, initialLogRows, totalRows, skippedFolders) => {
    const filterKeys = dedupe(jobActions.flatMap((action) => action.kind === "create" ? Object.keys(action.product.filterValues ?? {}) : []));
    const marketplaceFilterReferences = filterKeys.length > 0
    ...
    for (const action of jobActions) {
        ...
        try {
            if (action.kind === "existing") {
                ...
                const changed = mergedTypeMultiple.length !== existingState.customTypeMultiple.length;
                if (changed) {
            ...
            let logoSource = product.localLogoPath;
            ...
            try {
                const preparedLogoPath = product.localLogoPath !== null
                ...
                logoFileUrl = await uploadFileToShopify(preparedLogoPath, product.imageAltText);
                logoStatus = "uploaded";
            }
            catch (logoError) {
                reportNotes.push(`Logo unavailable: ${logoError instanceof Error ? logoError.message : String(logoError)}`);
            }
            await setShopifyMetafields(productId, product, logoFileUrl, marketplaceFilterReferences);
            ...
            if (isHandleAlreadyTakenError(error)) {
                const resolvedExisting = await resolveExistingProductAfterHandleConflict(currentCreateProduct);
                if (resolvedExisting) {
                    logRows.push({
    ...
    const attentionCsvPath = await writeAttentionCsvReport(logRows, jsonReport.timestamp);
    return {
        ...jsonReport,
        attentionCsvPath,
    };
...
const main = async () => {
    ...
    if (!shouldApply) {
        ...
        console.log(`Skipped existing in Shopify: ${report.summary.skipped_existing_shopify}`);
        console.log(`Skipped existing in current job: ${report.summary.skipped_existing_current_job}`);
        console.log(`Updated existing type_multiple: ${report.summary.updated_existing_type_multiple}`);
        console.log(`Skipped missing required data: ${report.summary.skipped_missing_required_data}`);
        console.log(`Skipped pricing unavailable: ${report.summary.skipped_pricing_unavailable}`);
        console.log(`Failed: ${report.summary.failed}`);
        console.log(`Action rows ready on apply: ${jobActions.length}`);
