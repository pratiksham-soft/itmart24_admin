...
const BATCH_LABEL = "software-batch11";
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-software-batch11-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "software";
const PREVIEW_FILE_PREFIX = "software-batch11-preview-";
    ...
    rows.forEach((row, index) => {
        ...
        row.collectionHandles.forEach((handle, collectionIndex) => {
            ...
            if (!TARGET_COLLECTION_HANDLES.has(handle)) {
                validationErrors.push(`${rowLabel} collection handle outside batch 11 scope: ${handle}`);
            }
...
main().catch((error) => {
    console.error("Software batch 11 upsert failed:", error);
    process.exitCode = 1;
});
