...
const BATCH_LABEL = "software-batch22";
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-software-batch22-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "software";
const PREVIEW_FILE_PREFIX = "software-batch22-preview-";
    ...
    rows.forEach((row, index) => {
        ...
        row.collectionHandles.forEach((handle, collectionIndex) => {
            ...
            if (!TARGET_COLLECTION_HANDLES.has(handle)) {
                validationErrors.push(`${rowLabel} collection handle outside batch 22 scope: ${handle}`);
            }
...
main().catch((error) => {
    console.error("Software batch 22 upsert failed:", error);
    process.exitCode = 1;
});
