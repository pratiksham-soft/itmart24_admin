...
import { formatAnalyticsConnectionError } from "../services/analyticsPostgres.service";
import { syncBlogJobSchedules } from "../services/blogJobScheduler.service";
import {
  createShopifyArticle,
  runAllActiveBlogJobs,
  runBlogJob,
} from "../services/blogAutomation.service";
import {
  createBlogJob,
  createBlogPost,
  createBlogTemplate,
...
import { listShopifyBlogs } from "../services/shopifyBlog.service";

const router = Router();

const toSafeRouteError = (error: unknown) =>
...
const sendDatabaseError = (
  ...
  const message = formatAnalyticsConnectionError(error);
...
router.put("/settings/ai-provider", async (req, res) => {
  try {
    const settings = await updateAIProviderSettings(req.body ?? {});
    res.json(settings);
  } catch (error) {
...
router.post("/jobs", async (req, res) => {
  try {
    const job = await createBlogJob(req.body ?? {});
    await syncBlogJobSchedules("job-created");
...
router.put("/jobs/:id", async (req, res) => {
  try {
    const job = await updateBlogJob(Number(req.params.id), req.body ?? {});
    await syncBlogJobSchedules("job-updated");
...
router.patch("/jobs/:id/status", async (req, res) => {
  try {
    const job = await toggleBlogJobStatus(
      Number(req.params.id),
      Boolean(req.body?.isActive)
    );
    await syncBlogJobSchedules("job-status-changed");
    console.log(
      `Blog job ${job?.status === "active" ? "activated" : "deactivated"} id=${job?.id ?? "unknown"} name="${job?.name ?? ""}"`
    );
    res.json(job);
  } catch (error) {
...
router.delete("/jobs/:id", async (req, res) => {
  try {
    await deleteBlogJob(Number(req.params.id));
    await syncBlogJobSchedules("job-deleted");
    res.status(204).send();
  } catch (error) {
...
router.post("/jobs/:id/run-once", async (req, res) => {
  try {
    ...
    const summary = await runBlogJob(Number(req.params.id), {
      triggerMode: "manual",
      requireActive: true,
    });
    res.json({
    ...
    const isInactiveError =
      safeError.includes("Only active blog jobs can run in this mode") ||
...
router.post("/jobs/run-active", async (_req, res) => {
  try {
    const summaries = await runAllActiveBlogJobs();
    res.json(summaries);
  } catch (error) {
    console.error("Active blog jobs run error:", error);
    res.status(400).json({
      error:
        error instanceof Error ? error.message : "Failed to run active blog jobs",
    });
  }
});
...
router.get("/shopify-blogs", async (_req, res) => {
  try {
    const blogs = await listShopifyBlogs();
    res.json(blogs);
  } catch (error) {
...
router.post("/templates", async (req, res) => {
  try {
    const template = await createBlogTemplate(req.body ?? {});
    res.status(201).json(template);
  } catch (error) {
    console.error("Blog template create error:", error);
    res.status(400).json({
      error:
        error instanceof Error ? error.message : "Failed to create blog template",
    });
  }
});
...
router.put("/templates/:id", async (req, res) => {
  try {
    const template = await updateBlogTemplate(Number(req.params.id), req.body ?? {});
    res.json(template);
  } catch (error) {
    console.error("Blog template update error:", error);
    res.status(400).json({
      error:
        error instanceof Error ? error.message : "Failed to update blog template",
    });
  }
});
...
router.delete("/templates/:id", async (req, res) => {
  try {
    await deleteBlogTemplate(Number(req.params.id));
    res.status(204).send();
  } catch (error) {
...
router.get("/blogs/:id", async (req, res) => {
  try {
    const blog = await getBlogPostById(Number(req.params.id));
...
router.put("/blogs/:id", async (req, res) => {
  try {
    const blog = await updateBlogPost(Number(req.params.id), req.body ?? {});
    res.json(blog);
  } catch (error) {
...
router.post("/posts/:id/publish", async (req, res) => {
  try {
    ...
    const result = await createShopifyArticle(Number(req.params.id), publish);
    res.json({
      success: true,
    ...
    res.status(400).json({
      ...
      error:
        error instanceof Error ? error.message : "Failed to create Shopify article",
    });
  }
});
...
router.post("/posts/publish-bulk", async (req, res) => {
  try {
    const ids = Array.isArray(req.body?.ids)
      ? req.body.ids.map((value: unknown) => Number(value)).filter(Number.isFinite)
      : [];

    const results = [];
    for (const id of ids) {
      results.push(await createShopifyArticle(id, true));
    }
    ...
    res.status(400).json({
      ...
      error:
        error instanceof Error ? error.message : "Failed to publish selected blog posts",
    });
  }
});
...
router.post("/blogs/:id/publish-shopify", async (req, res) => {
  try {
    const result = await createShopifyArticle(Number(req.params.id), Boolean(req.body?.publish));
    res.json(result);
  } catch (error) {
...
router.get("/todo-notes", (_req, res) => {
  res.json(getBlogManagerTodoNotes());
});
...
router.get("/logs", async (req, res) => {
  try {
    ...
    const logs = await listBlogJobRunLogs({
      jobId,
      limit,
    });
    res.json({
    ...
    res.status(500).json({
      ...
      error: formatAnalyticsConnectionError(error),
      logs: [],
    });
  }
});
