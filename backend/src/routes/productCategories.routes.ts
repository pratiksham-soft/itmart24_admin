 ...
 */
router.post("/main-category", async (req: Request, res: Response) => {
  try {
    const { name } = req.body;
    ...
    const docRef = await firestore.collection("product_categories").add({
      name,
      slug,
      isActive: true,
      isDeleted: false,
 ...
 * CREATE SUB CATEGORY
 */
router.post("/sub-category", async (req: Request, res: Response) => {
  try {
 ...
 */
router.post("/sub-sub-category", async (req: Request, res: Response) => {
  try {
      ...
      mains.map(async (main) => {
        ...
        await Promise.all(
          subs.map(async (sub: any) => {
            const subSubSnapshot = await firestore
              .collection("product_categories")
              .doc(main.id)
              .collection("subcategories")
              .doc(sub.id)
            ...
            sub.subsubcategories = subSubSnapshot.docs.map(
              (doc) => ({
                id: doc.id,
                ...doc.data(),
              })
 ...
 */
router.patch("/main-category/:id/delete", async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;

    const mainRef = firestore.collection("product_categories").doc(id);
 ...
 */
router.patch(
  "/sub-category/:mainId/:subId/delete",
  async (req: Request, res: Response) => {
    try {
      const mainId = req.params.mainId as string;
 ...
 */
router.patch(
  "/sub-sub-category/:mainId/:subId/:subSubId/delete",
  async (req: Request, res: Response) => {
    try {
      const mainId = req.params.mainId as string;
      const subId = req.params.subId as string;
      const subSubId = req.params.subSubId as string;
 ...
 */
router.patch("/main-category/:id", async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
 ...
 * UPDATE SUB CATEGORY
 */
router.patch("/sub-category/:mainId/:subId", async (req: Request, res: Response) => {
  try {
    const mainId = req.params.mainId as string;
    const subId = req.params.subId as string;
    const { name, isActive } = req.body;
 ...
 */
router.patch(
  "/sub-sub-category/:mainId/:subId/:subSubId",
  async (req: Request, res: Response) => {
    try {
      const mainId = req.params.mainId as string;
      const subId = req.params.subId as string;
 ...
 */
router.patch("/main-category/:id/restore", async (req: Request, res: Response) => {
  try {
    const id = req.params.id as string;
 ...
 */
router.patch(
  "/sub-category/:mainId/:subId/restore",
  async (req: Request, res: Response) => {
    try {
      const mainId = req.params.mainId as string;
      const subId = req.params.subId as string;
 ...
 */
router.patch(
  "/sub-sub-category/:mainId/:subId/:subSubId/restore",
  async (req: Request, res: Response) => {
    try {
      const mainId = req.params.mainId as string;
      const subId = req.params.subId as string;
