...
export class OpenAiBlogError extends Error {
...
const extractTextFromChoiceContent = (value: unknown) => {
      ...
      return (
        ...
        normalizeTextBlock(
          record.text &&
            ...
            "value" in (record.text as Record<string, unknown>)
            ? (record.text as Record<string, unknown>).value
            : null
        )
      );
    })
...
export const extractOpenAIText = (data: unknown) => {
        ...
        const textValue =
          ...
          normalizeTextBlock(
            blockRecord.text &&
              ...
              "value" in (blockRecord.text as Record<string, unknown>)
              ? (blockRecord.text as Record<string, unknown>).value
              : null
          ) ??
  ...
  const choiceContent =
    ...
    typeof record.choices[0] === "object"
      ? ((record.choices[0] as Record<string, unknown>).message as Record<
          string,
          unknown
        > | null)?.content
      : null;
  ...
  const choiceText =
    normalizeTextBlock(choiceContent) ?? extractTextFromChoiceContent(choiceContent);
  if (choiceText) {
    return choiceText;
  }
...
const ensureTags = (value: unknown) => {
  if (Array.isArray(value)) {
...
export const parseBlogJson = (rawText: string): GeneratedBlogContent => {
  let parsed: unknown;
  try {
...
export const classifyOpenAIError = (error: unknown) => {
  ...
  if (!axios.isAxiosError(error)) {
    ...
    const category: OpenAiErrorCategory =
  ...
  return {
    ...
    model:
      typeof data.model === "string" && data.model ? data.model : OPENAI_TEXT_MODEL,
  };
};

const logOpenAiParseFailure = (data: unknown) => {
  ...
  console.error("OpenAI response text extraction failed:", {
    ...
    debugTextPreview: OPENAI_BLOG_DEBUG ? extractOpenAIText(record) : undefined,
  });
};
  ...
  try {
    ...
    if (!outputText) {
      ...
      throw new OpenAiBlogError({
        ...
        model:
          typeof response.data?.model === "string" && response.data.model
            ? response.data.model
            : OPENAI_TEXT_MODEL,
      });
    }

    const content = parseBlogJson(outputText);
