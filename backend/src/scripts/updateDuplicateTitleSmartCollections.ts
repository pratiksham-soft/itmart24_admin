...
type UpdatedRecord = {
  ...
  status: "updated" | "already_configured";
};
...
const updateCollectionRuleSet = async ({
  collectionId,
  title,
  handle,
...
const isDesiredRuleSet = (
  ...
  return normalizedRuleKeys.every(
    (ruleKey, index) => ruleKey === expectedKeys[index]
  );
};

const waitForDesiredRuleSet = async ({
  collectionId,
  title,
  handle,
}: {
  ...
  throw new Error(
    `Timed out waiting for collection ${collectionId} to reflect the updated rule set`
  );
};
...
const main = async () => {
  ...
  const duplicateGroups = smartCollections
    ...
    .reduce<Map<string, ShopifyCollection[]>>((accumulator, collection) => {
      const key = normalizeKey(collection.title);
      const existing = accumulator.get(key) ?? [];
      existing.push(collection);
      accumulator.set(key, existing);
      return accumulator;
    }, new Map<string, ShopifyCollection[]>());
  ...
  const duplicateCollections = Array.from(duplicateGroups.values())
    ...
    .sort((left, right) => {
      const titleCompare = (left.collection.title ?? "").localeCompare(
        right.collection.title ?? ""
      );
      if (titleCompare !== 0) {
        return titleCompare;
      }
      return (left.collection.handle ?? "").localeCompare(
        right.collection.handle ?? ""
      );
    });

  console.log(
    `Found ${duplicateCollections.length} duplicate-title smart collections across ${Array.from(duplicateGroups.values()).filter((group) => group.length > 1).length} title groups.`
  );
  ...
  for (const item of duplicateCollections) {
    ...
    console.log(
      `Processing duplicate smart collection ${collectionId}: "${title}" -> "${handle}".`
    );
    ...
    const refreshedDetails =
      updateResult.jobId || !isDesiredRuleSet(updateResult.collection?.ruleSet, title, handle)
    ...
    if (!isDesiredRuleSet(refreshedDetails?.ruleSet, title, handle)) {
      skipped.push({
        ...
        duplicateCount,
        reason:
          "Collection update completed but the expected title+handle rule set was not present afterward",
      });
      continue;
    }
...
main()
  ...
  .catch((error) => {
    console.error(
      "Duplicate-title smart collection rule update failed:",
      error
    );
    process.exit(1);
  });
