...
type ShopifyProductRecord = {
  ...
  status: string;
  onlineStoreUrl: string | null;
  customUrlType: string | null;
  customUrlRaw: string | null;
  customUrl: string | null;
};

type UrlStatus = "working" | "unreachable";

type OfficialWebsiteAssessment =
...
type UrlProbeResult = {
  ...
  normalizedRequestedUrl: string | null;
  finalUrl: string | null;
  finalDomain: string;
  finalPath: string;
  httpStatus: number | null;
  urlStatus: UrlStatus;
  officialWebsiteAssessment: OfficialWebsiteAssessment;
...
const isNonOfficialDomain = (domain: string) =>
  NON_OFFICIAL_DOMAIN_PATTERNS.some((pattern) => pattern.test(domain));
...
const getRedirectCount = (response: AxiosResponse) => {
  const request = response.request as
    | {
        _redirectable?: {
...
const hasProductSchema = (html: string) =>
  /"@type"\s*:\s*"(?:product|[\w:]+product)"/i.test(
    html.toLowerCase()
  ) ||
...
const classifyProbeResult = ({
  ...
  const normalizedRequestedUrl =
    requestedUrl && isLikelyHttpUrl(requestedUrl)
  ...
  let officialWebsiteAssessment: OfficialWebsiteAssessment = "unclear";
  if (!normalizedRequestedUrl || !isLikelyHttpUrl(normalizedRequestedUrl)) {
    officialWebsiteAssessment = "invalid_url";
  } else if (
    areRelatedDomains(finalDomain || getHostname(normalizedRequestedUrl), getHostname(product.onlineStoreUrl)) ||
  ...
  let landingType: LandingType = "unreachable";
...
const fetchAllProducts = async () => {
  ...
  while (hasNextPage) {
    ...
    } = await withRetries("Fetch Shopify products for custom URL report", () =>
      shopifyGraphQL.post("", {
        query: `
    ...
    if (response.data?.errors?.length) {
      throw new Error(
        getGraphQlErrorMessage(
          ...
          "Failed to fetch Shopify products for the custom URL report"
        )
      );
    }
    ...
    nodes.forEach((node) => {
      ...
      products.push({
        ...
        onlineStoreUrl:
          normalizeWhitespace(node.onlineStoreUrl ?? "") || null,
        customUrlType:
          normalizeWhitespace(node.customUrlMetafield?.type ?? "") || null,
...
const probeProductUrl = async (
  ...
  try {
    const response = await withRetries(
      `Probe ${product.productId} ${requestedUrl}`,
      () =>
        axios.get<string>(requestedUrl, {
    ...
    const finalUrl = sanitizeUrl(
      getFinalResponseUrl(response, requestedUrl)
    );
    const contentType =
...
const writeReportFiles = async (rows: ReportRow[]) => {
  ...
  const summary = {
    ...
    officialWebsiteBreakdown: rows.reduce<Record<string, number>>(
      (accumulator, row) => {
...
const main = async () => {
  ...
  const urlTypeProducts = allProducts.filter(
    (product) => product.customUrlType?.toLowerCase() === "url"
  );
  ...
  console.log(
    `Found ${urlTypeProducts.length} products where custom.custom is typed as URL.`
  );

  if (selectedProducts.length === 0) {
    throw new Error(
      'No Shopify products were found with the "custom.custom" metafield typed as URL.'
    );
  }

  console.log(
    `Probing ${selectedProducts.length} URL${selectedProducts.length === 1 ? "" : "s"} with concurrency ${WORKER_CONCURRENCY}...`
  );
  ...
  const rows = await mapWithConcurrency(
    ...
    async (product, index) => {
      console.log(
        `[${index + 1}/${selectedProducts.length}] Checking ${product.productId}: ${product.title}`
      );
...
main().catch((error) => {
  console.error(
    "Failed to generate the Shopify custom URL report:",
    error?.message ?? error
  );
  process.exitCode = 1;
});
