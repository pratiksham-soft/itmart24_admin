...
const BATCH_LABEL = "software-batch44";
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-software-batch44-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "software";
const PREVIEW_FILE_PREFIX = "software-batch44-preview-";
    ...
    row.collectionHandles.forEach((handle, collectionIndex) => {
      ...
      if (!TARGET_COLLECTION_HANDLES.has(handle)) {
        validationErrors.push(
          `${rowLabel} collection handle outside batch 42 scope: ${handle}`
        );
      }
...
const applyDataset = async (rows: PreviewRow[]) => {
  ...
  const normalizeVerificationNotes = (
    ...
    const withoutPendingLogoNote = baseNotes.replace(
      /\s*(?:Logo processing and Shopify Files upload are still pending|Logo upload remains pending until the Shopify upsert script runs)\.?/gi,
      ""
    ).trim();
...
main().catch((error) => {
  console.error("Software batch 43 upsert failed:", error);
  process.exitCode = 1;
});
