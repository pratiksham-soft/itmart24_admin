...
const writePreflightReport = async (summary: ValidationSummary) => {
  ...
  const reportPath = path.join(
    EXPORTS_DIR,
    `cloud-services-phase1e-upload-preflight-${timestamp}.json`
  );
...
const upsertShopifyProduct = async (row: PreviewRow) => {
  ...
  const existingVariant = Array.isArray(existingProduct?.variants)
    ...
    : null;
  const primaryType =
    row.customTypeMultiple[0] ?? row.collectionTitles[0] ?? "Cloud Services";
...
const applyDataset = async (rows: PreviewRow[]) => {
  ...
  const reportPath = path.join(
    EXPORTS_DIR,
    `cloud-services-phase1e-upload-report-${timestamp}.json`
  );
