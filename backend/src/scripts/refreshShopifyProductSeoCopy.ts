...
const START_INDEX_ARG = process.argv.find((arg) =>
  arg.startsWith("--start-index=")
);
const START_INDEX = START_INDEX_ARG
  ? Math.max(0, Number(START_INDEX_ARG.split("=")[1]) || 0)
  : 0;
const SKIPPED_REPORT_ARG = process.argv.find((arg) =>
  arg.startsWith("--skipped-report=")
);
const PRODUCT_IDS_ARG = process.argv.find((arg) =>
  arg.startsWith("--product-ids=")
);
const MIN_WORDS_ARG = process.argv.find((arg) =>
  arg.startsWith("--min-words=")
);
...
const stripPlanModifiers = (value: string) =>
  normalizeWhitespace(
    value.replace(
      /\b(beginner|enterprise|professional|turbo|starter|basic|premium|pro|plus|business|standard|advanced)\b/gi,
      " "
    )
  );
...
const pickVariant = <T>(values: T[], seed: number) =>
  values[seed % values.length];

const resolveInputPath = (value: string) =>
  path.isAbsolute(value) ? value : path.resolve(process.cwd(), value);

const loadProductIdsFromSkippedReport = (reportArg: string | undefined) => {
  if (!reportArg) {
    return null;
  }
  ...
  const payload = JSON.parse(fs.readFileSync(reportPath, "utf8")) as {
    skipped?: Array<{ productId?: number | string }>;
    updates?: Array<{ productId?: number | string; updated?: boolean }>;
  };
...
const fetchAllProducts = async () => {
  ...
  while (hasNextPage) {
    ...
    } = await withRetries("Fetch Shopify products for SEO refresh", () =>
      shopifyGraphQL.post("", {
        query: `
    ...
    if (response.data?.errors?.length) {
      throw new Error(
        getGraphQlErrorMessage(
          ...
          "Failed to fetch Shopify products for SEO refresh"
        )
      );
    }
...
const buildSelectionSummary = (products: ShopifyProductRecord[]) => {
  const activeProducts = products.filter(
    (product) => product.status.toLowerCase() === "active"
  );
  const withCustomUrl = activeProducts.filter((product) =>
    isLikelyHttpUrl(product.customUrl)
  );
...
const collectJsonLdStrings = (value: unknown, keyHint = ""): string[] => {
  ...
  if (value && typeof value === "object") {
    return Object.entries(value).flatMap(([key, nestedValue]) =>
      collectJsonLdStrings(nestedValue, key)
    );
  }
...
const filterContentLine = (value: string) => {
  const normalized = normalizeWhitespace(value);
  if (!normalized) {
    return false;
  }
...
const extractMetaContent = (html: string, attr: string, name: string) => {
...
const extractSourceProfile = (url: string, html: string): SourceProfile => {
  ...
  const metaDescription =
    extractMetaContent(cleanedHtml, "name", "description") ||
  ...
  const paragraphs = extractMatchedText(
    cleanedHtml,
    /<p[^>]*>[\s\S]*?<\/p>/gi,
    18
  ).slice(0, 12);

  const featureBullets = extractMatchedText(
    cleanedHtml,
    /<li[^>]*>[\s\S]*?<\/li>/gi,
    30
  )
  ...
  const jsonLdStrings = toUniqueList(
    jsonLdMatches.flatMap((block) => {
      const inner =
        block.match(
...
const sourceProfileCache = new Map<string, SourceProfile | null>();
const searchCache = new Map<string, SearchResult[]>();

const fetchSourceProfile = async (url: string) => {
...
const decodeBingUrl = (href: string) => {
  ...
  if (normalizedHref.startsWith("http://") || normalizedHref.startsWith("https://")) {
    ...
    if (encoded.startsWith("a1")) {
      try {
        return sanitizeUrl(
          Buffer.from(encoded.slice(2), "base64").toString("utf8")
        );
      } catch {
...
const getSearchIdentityTokens = (product: ShopifyProductRecord) =>
  tokenize(extractCoreProductName(product.title)).filter(
    (token) => token.length >= 3 && !GENERIC_SEARCH_TOKENS.has(token)
  );

const getVendorSearchTokens = (product: ShopifyProductRecord) =>
  tokenize(product.vendor).filter((token) => token.length >= 3);

const getSearchContextTokens = (product: ShopifyProductRecord) => {
  ...
  return tokenize(
    [product.title, product.productType, product.tags.join(" ")].join(" ")
  )
    .filter(
      (token) =>
        token.length >= 4 &&
...
const isTrustedSearchDomain = (domain: string) =>
  TRUSTED_DOMAIN_PATTERNS.some((pattern) => pattern.test(domain));
...
const isHighConfidenceSearchDomain = ({
  domain,
  preferredDomain,
}: {
  domain: string;
  preferredDomain: string;
}) => areRelatedDomains(domain, preferredDomain) || isTrustedSearchDomain(domain);
...
const isRelevantSearchResult = ({
  ...
  const haystack = normalizeKey(
    [result.title, result.snippet, result.domain].join(" ")
  );
  ...
  const matchedVendorTokens = vendorTokens.filter(
    (token) => haystack.includes(token) || result.domain.includes(token)
  );
    ...
    resultMatches
      ...
      .map((chunk) => {
        ...
        const snippet =
          ...
          chunk.match(
            /class="result__snippet"[\s\S]*?>([\s\S]*?)<\/div>/i
          )?.[1] ??
          "";
...
const searchBing = async (query: string) => {
  ...
  const resultRegex =
    /<h2[^>]*><a[^>]+href="([^"]+)"[^>]*>([\s\S]*?)<\/a><\/h2>[\s\S]{0,1800}?(?:<div class="b_caption"[\s\S]*?<p[^>]*>([\s\S]*?)<\/p>|<p[^>]*>([\s\S]*?)<\/p>)/gi;
...
const mergeSourceProfiles = (
  ...
  return {
    ...
    metaDescription:
      ...
      "",
    headings: toUniqueList(profiles.flatMap((profile) => profile.headings)).slice(
      0,
      16
    ),
    paragraphs: toUniqueList(
      profiles.flatMap((profile) => profile.paragraphs)
    ).slice(0, 16),
    featureBullets: toUniqueList(
      profiles.flatMap((profile) => profile.featureBullets)
    ).slice(0, 14),
    text: mergedText,
...
const fetchProfilesFromResults = async (
...
const filterRelevantSearchResults = ({
  results,
  product,
  preferredDomain,
}: {
...
const hasSufficientSourceDepth = (
  ...
  const haystack = normalizeKey(
    [profile.pageTitle, profile.metaDescription, profile.text].join(" ")
  );
  ...
  return (
    matchedTokens.length >= Math.min(2, productTokens.length) &&
    profile.wordCount >= 35
  );
};

const extractSearchSnippetProfile = (
  results: SearchResult[],
  fallbackUrl: string
): SourceProfile | null => {
  const relevantResults = results.slice(0, 5);
  const text = toUniqueList(
    relevantResults.flatMap((result) => [result.title, result.snippet])
  ).join(" ");
...
const resolveSourceForProduct = async (product: ShopifyProductRecord) => {
  ...
  const runFallbackSearch = async (query: string) => {
    try {
      ...
      console.warn(
        `[search:error] ${product.productId} "${query}" -> ${error?.message ?? "unknown error"}`
      );
      return [];
    }
  };
  ...
  if (isLikelyHttpUrl(product.customUrl)) {
    const directProfile = await fetchSourceProfile(product.customUrl as string);
    if (
      directProfile &&
      (CUSTOM_URL_ONLY ||
      ...
      return {
        sourceMethod: "custom metafield" as const,
    ...
    if (CUSTOM_URL_ONLY) {
      return {
        sourceMethod: "insufficient" as const,
        sourceUrl: product.customUrl,
        profile: null,
      };
    }
    ...
    if (preferredDomain) {
      const domainResultsMap = new Map<string, SearchResult>();
      ...
      const domainProfiles = await fetchProfilesFromResults(
        relevantDomainResults,
        product,
        preferredDomain
      );
      const mergedDomainProfile = mergeSourceProfiles(
        domainProfiles,
        product.customUrl as string
      );

      if (hasSufficientSourceDepth(mergedDomainProfile, product, preferredDomain)) {
        return {
          sourceMethod:
            domainProfiles.length > 1 ? ("multi-source web" as const) : ("domain search" as const),
          sourceUrl:
            domainProfiles[0]?.url ??
  ...
  const webResultsMap = new Map<string, SearchResult>();
  ...
  const highConfidenceWebResults = webResults.filter((result) =>
    isHighConfidenceSearchDomain({
      domain: result.domain,
      preferredDomain,
    })
  );
  const preferredDomainWebResults = highConfidenceWebResults.filter((result) =>
    areRelatedDomains(result.domain, preferredDomain)
  );
  const prioritizedWebResults =
    preferredDomainWebResults.length > 0
      ? preferredDomainWebResults
      : highConfidenceWebResults;

  const snippetProfile = extractSearchSnippetProfile(
    prioritizedWebResults,
    prioritizedWebResults[0]?.url ?? product.customUrl ?? ""
  );
  ...
  if (
    ...
    return {
      sourceMethod: "search snippets" as const,
      sourceUrl: snippetProfile.url || null,
      profile: snippetProfile,
    };
  }

  const webProfiles = await fetchProfilesFromResults(
    prioritizedWebResults,
    product,
    preferredDomain
  );
  const mergedWebProfile = mergeSourceProfiles(
    webProfiles,
    prioritizedWebResults[0]?.url ?? product.customUrl ?? ""
  );
  ...
  if (hasSufficientSourceDepth(mergedWebProfile, product, preferredDomain)) {
    return {
      sourceMethod:
        webProfiles.length > 1 ? ("multi-source web" as const) : ("web search" as const),
      sourceUrl:
        webProfiles[0]?.url ??
...
const inferProductFamily = (product: ShopifyProductRecord) => {
  const combined = normalizeKey(
    [product.title, product.productType, product.tags.join(" ")].join(" ")
  );

  if (
    /\b(hosting|server|cloud|wordpress|infrastructure|vps|dedicated)\b/.test(
      combined
    )
  ) {
...
const humanizeCategory = (product: ShopifyProductRecord) =>
  product.productType ||
...
const buildFeaturePool = (
  ...
  return rawFeatures
    .filter((feature) => {
      ...
      const tokenHits = tokenize(product.title).filter((token) =>
        normalizeKey(feature).includes(token)
      );
...
const buildUseCaseSentences = ({
  ...
  if (family === "infrastructure") {
    sentences.push(
      `${product.title} is most relevant for ${audience}, especially when uptime, delivery speed, and manageable scaling costs are part of the buying conversation.`
    );
    sentences.push(
      `In infrastructure categories, the difference between a basic service and a production-ready platform usually comes down to consistency under load, operational controls, and the amount of hands-on maintenance a team has to absorb.`
    );
  } else if (family === "ai") {
    sentences.push(
      `${product.title} makes the strongest case for ${audience} that want automation, analysis, or generation features while still keeping workflow quality and oversight in view.`
    );
    sentences.push(
    ...
    sentences.push(
      `${product.title} is likely to appeal to ${audience} that need stronger output, faster campaign execution, and clearer content workflows without expanding headcount for every task.`
    );
    sentences.push(
      `In marketing software, the real test is whether the platform shortens production cycles, supports repeatable quality, and gives teams enough control to keep messaging aligned across channels.`
    );
  } else if (family === "analytics") {
    sentences.push(
      `${product.title} fits best with ${audience}, particularly when scattered data, slow reporting, or hard-to-interpret performance signals are making decisions more reactive than strategic.`
    );
    sentences.push(
      `Analytics products tend to stand out when they can turn raw information into usable direction for planning, prioritization, and faster course correction.`
    );
  } else if (family === "security") {
    sentences.push(
      `${product.title} is positioned well for ${audience} that need stronger policy visibility, more reliable controls, and a clearer operational view of risk.`
    );
    sentences.push(
      `Security and governance platforms earn trust when they reduce ambiguity, document important decisions, and help teams respond with more confidence when requirements change.`
    );
  } else {
    sentences.push(
      `${product.title} reads like a ${category} built for ${audience}, with the strongest value coming from how easily it can fit into routine business work.`
    );
    sentences.push(
      `For general software categories, buyers often compare products based on usability, operational clarity, and how quickly the tool can become useful after implementation starts.`
    );
  }

  if (clues.includes("integration")) {
    sentences.push(
      `That positioning becomes more convincing when integration language appears in the source material, because it suggests the product is designed to sit inside an existing stack instead of forcing teams to rebuild their process around it.`
    );
  }
...
const padBodyText = ({
  ...
  while (countWords(result.join(" ")) < MIN_WORDS) {
    ...
    if (profile.metaDescription) {
      result.push(
        `The official messaging around ${product.title} suggests that the product is being framed with a specific operational promise, which is helpful for buyers who want to compare stated value against the workflows and feature signals they see elsewhere on the site.`
      );
    }
  }
...
const truncateSmart = (value: string, maxLength: number) => {
  ...
  return normalizeWhitespace(
    `${trimmed.slice(0, lastSpace > 40 ? lastSpace : maxLength).trim()}`
  );
};
...
const updateShopifyProduct = async ({
  ...
  await withRetries(`Update Shopify product ${product.productId}`, () =>
    shopifyRest.put(`/products/${product.productId}.json`, {
      product: {
...
const writeReportFiles = async (updates: ProductUpdatePreview[]) => {
  await fs.promises.mkdir(EXPORTS_DIR, { recursive: true });

  const timestamp = new Date().toISOString().replace(/[:.]/g, "-");
  const baseName = `shopify-product-seo-refresh-${timestamp}${
...
const main = async () => {
  ...
  const offsetProducts =
    START_INDEX > 0 ? idFilteredProducts.slice(START_INDEX) : idFilteredProducts;
  ...
  console.log(
    `Completed ${updates.length} products. Updated: ${
      updates.filter((item) => item.updated).length
    }.`
  );
};
