...
const BATCH_LABEL = "ai-tools-ar-behavior";
const LOGO_TEMP_DIR = path.resolve(EXPORTS_DIR, "tmp-ai-tools-ar-behavior-logos");
const SHOPIFY_GRAPHQL_PAGE_SIZE = 100;
const TARGET_CATEGORY_SLUG = "ai-tools";
const PREVIEW_FILE_PREFIX = "ai-tools-ar-behavior-preview-";
const REQUIRED_COLLECTION_HANDLES = new Set([
  "ar-navigation-ai",
  "ar-training-simulations",
  "behavioral-analysis-ai",
]);
const ALLOWED_COLLECTION_HANDLES = new Set([
  "ar-navigation-ai",
  "ar-training-simulations",
  "behavioral-analysis-ai",
  "training-software",
...
const validatePreviewRows = async (
  ...
  rows.forEach((row, index) => {
    ...
    row.collectionHandles.forEach((handle, collectionIndex) => {
      ...
      if (!ALLOWED_COLLECTION_HANDLES.has(handle)) {
        validationErrors.push(
          `${rowLabel} collection handle outside ai-tools ar-behavior scope: ${handle}`
        );
      }
  ...
  REQUIRED_COLLECTION_HANDLES.forEach((handle) => {
...
main().catch((error) => {
  console.error("AI tools AR/behavior upsert failed:", error);
  process.exitCode = 1;
});
