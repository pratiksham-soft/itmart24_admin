...
const main = async () => {
  ...
  const jsonPath = path.join(EXPORTS_DIR, `cloud-services-phase1e-preview-${timestamp}.json`);
  const csvPath = path.join(EXPORTS_DIR, `cloud-services-phase1e-preview-${timestamp}.csv`);
  const reportPath = path.join(EXPORTS_DIR, `cloud-services-phase1e-validation-${timestamp}.json`);
  ...
  const validation = {
    ...
    phase1dPreviewImported: path.basename(phase1dPath),
