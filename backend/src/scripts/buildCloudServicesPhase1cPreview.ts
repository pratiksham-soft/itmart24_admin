...
const readJson = async <T>(filePath: string) =>
  JSON.parse(await fs.promises.readFile(filePath, "utf8")) as T;
...
const latestExportPath = (prefix: string, suffix: string) => {
  const file = fs
...
const buildPreviewRow = (
  spec: Spec,
  cloudByTitle: Map<string, string>,
  defs: Map<string, Def>
): Preview => {
...
const buildPreviewCsv = (rows: Preview[]) => {
  const headers = [
...
const main = async () => {
  ...
  const [categoryRows, filterRows, zeroRows, phase1Rows, phase1bRows] = await Promise.all([
    ...
    readJson<Preview[]>(phase1Path),
    readJson<Preview[]>(phase1bPath),
  ]);
  ...
  const mergedByHandle = new Map<string, Preview>();
  ...
  const jsonPath = path.join(EXPORTS_DIR, `cloud-services-phase1c-preview-${timestamp}.json`);
  const csvPath = path.join(EXPORTS_DIR, `cloud-services-phase1c-preview-${timestamp}.csv`);
  const reportPath = path.join(EXPORTS_DIR, `cloud-services-phase1c-validation-${timestamp}.json`);
  ...
  const validation = {
    ...
    zeroCollectionsExportUsed: path.basename(zeroCollectionsPath),
    phase1PreviewImported: path.basename(phase1Path),
    phase1bPreviewImported: path.basename(phase1bPath),
    totalZeroCloudCollectionsAtStart: zeroCloud.length,
  };

  await Promise.all([
    fs.promises.writeFile(jsonPath, JSON.stringify(allRows, null, 2), "utf8"),
    fs.promises.writeFile(csvPath, buildPreviewCsv(allRows), "utf8"),
