...
const buildBodyHtml = () => {
  const featureSentence = joinAsSentence(CS_CART_B2B_OVERRIDE.featureLines.slice(0, 4));
  const pricingSentence = joinAsSentence(CS_CART_B2B_OVERRIDE.pricingLines.slice(0, 4));

  return [
    `<p>${CS_CART_B2B_OVERRIDE.title} is positioned as a hosted marketplace builder for businesses that want B2B commerce workflows without taking on infrastructure management. The official CS-Cart cloud offer focuses on helping teams launch a multi-vendor marketplace quickly, manage sellers and products through an admin interface, and avoid a custom development cycle for core platform setup. That makes the product relevant for organizations comparing B2B marketplace platforms that need faster launch speed, clearer operating controls, and a managed delivery model.</p>`,
  ...
  const metafields = [
    {
      ...
      key: "custom",
      type: "url",
      value: CS_CART_B2B_OVERRIDE.officialUrl,
    },
    {
      ownerId: PRODUCT_GID(productId),
      namespace: "custom",
...
const main = async () => {
  ...
  if (!product?.id) {
    throw new Error(`Could not find Shopify product with handle ${CS_CART_B2B_HANDLE}`);
  }

  const primaryVariant = Array.isArray(product.variants) ? product.variants[0] : null;
  if (!primaryVariant?.id) {
    throw new Error(`Could not find primary variant for ${CS_CART_B2B_HANDLE}`);
  }

  await shopifyRest.put(`/products/${product.id}.json`, {
    product: {
      id: product.id,
      body_html: buildBodyHtml(),
      metafields_global_title_tag: CS_CART_B2B_OVERRIDE.seoTitle,
      metafields_global_description_tag: CS_CART_B2B_OVERRIDE.seoDescription,
      variants: [
        {
          id: primaryVariant.id,
          price: String(CS_CART_B2B_OVERRIDE.price),
          taxable: false,
  ...
  console.log(`Updated ${CS_CART_B2B_HANDLE} (${product.id})`);
  console.log(`Official URL: ${CS_CART_B2B_OVERRIDE.officialUrl}`);
  console.log(`Price: ${CS_CART_B2B_OVERRIDE.price}`);
};
