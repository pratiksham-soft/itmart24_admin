...
const main = async () => {
  ...
  const jsonPath = path.join(EXPORTS_DIR, `cloud-services-phase1d-preview-${timestamp}.json`);
  const csvPath = path.join(EXPORTS_DIR, `cloud-services-phase1d-preview-${timestamp}.csv`);
  const reportPath = path.join(EXPORTS_DIR, `cloud-services-phase1d-validation-${timestamp}.json`);
  ...
  const validation = {
    ...
    phase1cPreviewImported: path.basename(phase1cPath),
