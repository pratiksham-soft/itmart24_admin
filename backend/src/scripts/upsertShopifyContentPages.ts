...
import { resolvePricingDetails } from "../services/subscriptionPlanPricing";
...
const SUBSCRIPTION_VENDOR_PRICING_ENDPOINT =
  "https://shavi.itmart24.com/api/subscription-plans/vendor-pricing";
...
const renderDiscountBadge = (discountPercentage: number) =>
  discountPercentage > 0 ? `${discountPercentage}% OFF` : "";

const renderSaveLabel = (saveAmount: number | null, currencyCode: string) =>
  saveAmount && saveAmount > 0
    ? `Save ${formatPrice(saveAmount, currencyCode)}`
    : "";

const getDefaultPricingIndex = (
  options: Array<Pick<VendorPagePeriod, "isDiscounted" | "discountPercentage">>
) => {
  const discountedIndex = options.findIndex(
    (option) => option.isDiscounted || option.discountPercentage > 0
  );
...
const normalizeVendorPagePlan = (
  ...
  index: number,
  targetCountryCode: string | null = null
): VendorPagePlan | null => {
  ...
  const monthlyReferencePeriod =
    rawPeriods.find((period) =>
  ...
  const periods = rawPeriods
    ...
    .map((period, periodIndex) => {
      ...
      return {
        ...resolvePricingDetails(period, targetCountryCode, monthlyReferencePeriod),
          ...
          (period, index) => `
            ...
            >
              ...
              <div class="vendor-pricing-page__price-meta-top">
                <span class="vendor-pricing-page__original-price ${period.isDiscounted ? "" : "is-hidden"}" data-price-original>${escapeHtml(
                  formatPrice(period.originalPrice, period.currencyCode)
                )}</span>
                <span class="vendor-pricing-page__discount-badge ${period.isDiscounted ? "" : "is-hidden"}" data-price-discount>${escapeHtml(
                  renderDiscountBadge(period.discountPercentage)
                )}</span>
              </div>
              <div class="vendor-pricing-page__price-row">
                <span class="vendor-pricing-page__main-price" data-price-current>${escapeHtml(
                  formatPrice(period.discountedPrice, period.currencyCode)
                )}</span>
                <span class="vendor-pricing-page__price-term">${escapeHtml(period.termLabel)}</span>
              </div>
              <div class="vendor-pricing-page__save-row ${period.saveAmount ? "" : "is-hidden"}" data-price-save>${escapeHtml(
                renderSaveLabel(period.saveAmount, period.currencyCode)
              )}</div>
            </div>
          `
        )
      ...
      <div class="vendor-pricing-page__portfolio-ranges">
        ${group.portfolioPlans
          .map((portfolioPlan, portfolioIndex) => {
            ...
            const defaultPricingIndex = getDefaultPricingIndex(
                      ...
                      (option, optionIndex) => `
                        <div
                          class="vendor-pricing-page__price-panel ${
                            optionIndex === defaultPricingIndex ? "is-active" : ""
                          }"
                        ...
                        >
                          ...
                          <div class="vendor-pricing-page__price-row">
                            <span class="vendor-pricing-page__main-price" data-price-current>${escapeHtml(
                              formatPrice(option.discountedPrice, option.currencyCode)
                            )}</span>
                          </div>
...
type ClientPricingDetails = Pick<
  VendorPagePeriod,
  | "currencyCode"
  | "originalPrice"
  | "discountPercentage"
...
type ClientPricingPayload = {
  plans: Array<{
    id: string;
    periods: Array<{
      id: string;
      pricing: ClientPricingDetails;
    }>;
    portfolioPlans: Array<{
      id: string;
...
const toClientPricingDetails = (
  pricing: VendorPagePeriod | VendorPortfolioPricingOption
...
const buildClientPricingPayload = (
  rawPlans: RawSubscriptionPlan[],
  targetCountryCode: string | null
): ClientPricingPayload => ({
  plans: rawPlans
    .map((rawPlan, index) => {
      ...
      const portfolioGroup = normalizeVendorPortfolioGroup(
        rawPlan,
        plan,
        targetCountryCode
      );
      ...
      return {
        ...
        portfolioPlans: (portfolioGroup?.portfolioPlans ?? []).map(
          (portfolioPlan) => ({
            id: portfolioPlan.id,
    ...
    .filter(
      (
        entry
      ): entry is ClientPricingPayload["plans"][number] => Boolean(entry)
    ),
});
...
const buildVendorPageHtml = (
  ...
  portfolioGroups: VendorPortfolioGroup[],
  clientPricingPayloads: {
    global: ClientPricingPayload;
    india: ClientPricingPayload;
  }
) => `
  <style>
    .main-page-title.page-title {
...
const loadVendorPageSpec = async (): Promise<PageSpec> => {
  const { plans, portfolioGroups, clientPricingPayloads } =
    await loadActiveVendorPricingData();
