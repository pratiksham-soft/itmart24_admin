    ...
    <label className="block space-y-2.5">
      ...
      <div className="relative">
        <input
          ...
          disabled={disabled}
          className={[
            "portal-input bg-white text-slate-950",
            error ? "border-rose-400 text-rose-700 focus:border-rose-400 focus:ring-4 focus:ring-rose-500/15" : "",
          ].join(" ")}
        />

        {open && !disabled ? (
          <div className="absolute z-20 mt-2 max-h-60 w-full overflow-y-auto rounded-[1.15rem] border border-slate-200 bg-white p-2 shadow-[0_18px_50px_-24px_rgba(15,23,42,0.35)]">
            {loading ? (
              ...
              filteredOptions.map((option, index) => (
                <button
                  ...
                  className={[
                    ...
                    activeIndex === index || value === option.value ? "bg-sky-50 text-sky-700" : "text-slate-700 hover:bg-slate-50",
                  ].join(" ")}
                >
                  <span className="truncate">{option.label}</span>
                  {value === option.value ? <span className="text-xs font-semibold uppercase tracking-[0.16em]">Selected</span> : null}
                </button>
              ))
            ) : (
              <p className="px-3 py-2 text-sm text-slate-500">No matching options found.</p>
            )}
          </div>
        ) : null}
      </div>
      {error ? <span className="block text-xs leading-6 text-rose-300">{error}</span> : null}
    </label>
  );
}
...
export function MyReviewsPage() {
  ...
  const editingReview = useMemo(
    () => reviews?.find((item) => item.id === editingReviewId) ?? null,
  ...
  async function startEdit(review: ReviewRecord) {
    ...
    setCategoryQuery(String(metadata.selectedTopCategoryLabel ?? ""));
    setSubcategoryQuery(String(metadata.selectedSubcategoryLabel ?? ""));
    setCollectionQuery(String(metadata.selectedCollectionLabel ?? ""));
    setProductQuery(review.productName);
  ...
  function handleTopCategoryChange(nextTopCategory: string) {
    ...
    setCategoryQuery(nextTopCategory ? topCategoryOptions.find((item) => item.value === nextTopCategory)?.label ?? "" : "");
    setSubcategoryQuery("");
    setCollectionQuery("");
    setProductQuery("");
    setCollectionProducts([]);
  ...
  function handleSubcategoryChange(nextSubcategory: string) {
    ...
    setSubcategoryQuery(nextSubcategory ? subcategoryOptions.find((item) => item.value === nextSubcategory)?.label ?? "" : "");
    setCollectionQuery("");
    setProductQuery("");
    setCollectionProducts([]);
  }
  ...
  async function handleCollectionChange(nextCollectionHandle: string) {
    ...
    setCollectionQuery(nextCollectionHandle ? collectionOptions.find((item) => item.value === nextCollectionHandle)?.label ?? "" : "");
    setProductQuery("");
  ...
  function handleCategoryInputChange(nextValue: string) {
  ...
  function handleSubcategoryInputChange(nextValue: string) {
    setSubcategoryQuery(nextValue);
    setCollectionQuery("");
    setProductQuery("");
    setCollectionProducts([]);
    setForm((current) => ({
  ...
  function handleCollectionInputChange(nextValue: string) {
    setCollectionQuery(nextValue);
    setProductQuery("");
    setCollectionProducts([]);
    setForm((current) => ({
      ...current,
  ...
  function handleProductInputChange(nextValue: string) {
  ...
  const categoryError = categoryQuery.trim() && !form.selectedTopCategory ? "Select a valid category from the list." : "";
  const subcategoryError = subcategoryQuery.trim() && !form.selectedSubcategory ? "Select a valid sub category from the list." : "";
  const collectionError = collectionQuery.trim() && !form.selectedCollectionHandle ? "Select a valid collection from the list." : "";
  const productError = productQuery.trim() && !form.productHandle ? "Select a valid product from the list." : "";
  const reviewSelectionValid = Boolean(
        ...
        <aside className="space-y-6">
          <form ref={formRef} onSubmit={submitReview} className="rounded-[34px] border border-white/10 bg-[#050a1d] p-6 text-white shadow-[0_40px_110px_-70px_rgba(2,6,23,0.95)]">
            <div className="flex items-start gap-3">
              ...
              <div>
                <h2 className="text-2xl font-semibold">{editingReview ? "Edit strategic review" : "Create strategic review"}</h2>
            ...
            <div className="mt-6 grid gap-4 md:grid-cols-2">
              <SearchableSelect
                ...
                onSelect={(option) => handleTopCategoryChange(option.value)}
              ...
              <SearchableSelect
                ...
                placeholder={form.selectedTopCategory ? "Type to search sub category" : "Select category first"}
              ...
              <SearchableSelect
                ...
                onSelect={(option) => void handleCollectionChange(option.value)}
                placeholder={form.selectedSubcategory ? "Type to search collection" : "Select sub category first"}
              ...
              <SearchableSelect
                ...
                options={collectionProducts.map((item) => ({ value: item.productHandle, label: item.productName }))}
                onInputChange={handleProductInputChange}
                onSelect={(option) => handleProductChange(option.value)}
                placeholder={form.selectedCollectionHandle ? "Type to search product" : "Select collection first"}
                disabled={!form.selectedCollectionHandle}
              ...
              <ReadonlyField label="Product ID" value={form.shopifyProductId} />
                ...
                {category.dimensions.map((dimension) => (
                  <label key={dimension.key} className="rounded-[22px] border border-white/10 bg-slate-950/50 px-4 py-3">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <span className="text-sm font-semibold text-sky-300">{(form.ratings[dimension.key] ?? 5).toFixed(1)}</span>
                    </div>
                    <input
                      type="range"
                      min="1"
            ...
            <div className="mt-6 flex flex-wrap gap-3">
              ...
              >
                ...
                {isSubmitting ? "Saving..." : editingReview ? "Update review" : "Publish review"}
              </button>
