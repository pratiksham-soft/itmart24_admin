...
const RejectedProducts = () => {
  ...
  const fetchRejectedProducts = async () => {
    ...
    try {
      const res = await fetch("http://localhost:5000/api/products/rejected");
  ...
  const {
    ...
    onSuccess: fetchRejectedProducts,
  });
  ...
  if (loading) {
    return <div>Loading rejected products...</div>;
  }

  return (
    <div className="space-y-6">
      <ComponentCard title="Rejected Products">
        <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
          <ProductSearchBar
            id="rejected-products-search"
            value={searchQuery}
        ...
        {filteredProducts.length === 0 ? (
          <p className="text-sm text-gray-500">
            {searchQuery
              ? "No rejected products match your search."
