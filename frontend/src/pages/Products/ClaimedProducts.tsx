...
const ClaimedProducts = () => {
  ...
  const fetchClaimedProducts = async () => {
    ...
    try {
      const response = await fetch("http://localhost:5000/api/products/claimed");
      ...
      console.error("Failed to load claimed products", error);
    } finally {
      setLoading(false);
    }
  };
  ...
  const {
    ...
    onSuccess: fetchClaimedProducts,
  });
  ...
  if (loading) {
    return <div className="text-sm text-gray-500">Loading claimed products...</div>;
  }
