...
const PlanForm = ({ plan, onClose, onSaved }: PlanFormProps) => {
  ...
  const updatePeriod = (
    index: number,
    field: "label" | "durationInMonths" | "price" | "discountPercentage",
    value: string | number
  ) => {
    setForm((prev) => {
  ...
  const handleFeatureDrop = (event: DragEvent<HTMLDivElement>, targetIndex: number) => {
    event.preventDefault();
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const response = form.id
        ? await axios.put(`/api/user-plans/${form.id}`, payload)
        : await axios.post("/api/user-plans", payload);
                ...
                <div className="grid gap-4 lg:grid-cols-[minmax(0,1.2fr)_minmax(0,0.8fr)]">
                  <div className="space-y-4">
                    ...
                    <div>
                      ...
                      <textarea
                        className="w-full rounded-2xl border border-gray-300 bg-white px-4 py-3 text-sm text-gray-900 outline-none transition focus:border-brand-400 dark:border-gray-700 dark:bg-gray-950 dark:text-white"
                        placeholder="Short summary for the plans page"
                        rows={4}
                  ...
                  <div className="rounded-2xl border border-sky-100 bg-sky-50/70 px-4 py-4 dark:border-sky-900/40 dark:bg-sky-950/20">
                    ...
                    <p className="mt-2 text-sm text-sky-800/80 dark:text-sky-200/80">
                      Each billing period supports one default price plus optional
                      market-specific overrides with independent discounts.
                    </p>
                  </div>
                </div>
              </div>
                ...
                <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                  <div>
                    ...
                    <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                      Keep one default billing price, then localize by market only
                      where needed.
                    </p>
                  </div>
